...
static int comp_angle(image_t *img, kp_t *kp, float *a, float *b)
{
    ...
    int angle = (int) (fast_atan2f((float)m_01, (float)m_10) * (180.0f/M_PI));
    if (angle < 0) {
        angle += 360;
    }
...
array_t *orb_find_keypoints(image_t *img, int threshold, int max_keypoints)
{
	...
	// Find kpts
    fast_detect(img, kpts, threshold, max_keypoints, true);
	if(array_length(kpts) == 0)
	{
	    array_free(kpts);
        kpts = NULL;
		return NULL;
    ...
    for (int k=kpts_index; k<array_length(kpts); k++, kpts_index++) {
        ...
        kpt->angle = comp_angle(img, kpt, &a, &b);

        #define GET_VALUE(idx) \
               (x = (int) fast_roundf(pattern[idx].x*a - pattern[idx].y*b), \
                y = (int) fast_roundf(pattern[idx].x*b + pattern[idx].y*a), \
                img->pixels[((kpt->y+y)*img->w)+(kpt->x+x)])
...
int orb_filter_keypoints(array_t *kpts, rectangle_t *r, point_t *c)
{
    ...
    float *kpts_dist = xalloc0(kpts_size * sizeof(float));
    ...
    for (int i=0; i<kpts_size; i++) {
        kp_t *kp = array_at(kpts, i);
        if (kp->matched) {
            float dist = fast_fabsf(mdist - kpts_dist[i]);
            if (dist > stdist) {
                kp->matched = 0;
            } else {
                int x, y;
