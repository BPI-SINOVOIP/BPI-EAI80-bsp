...
typedef enum
{
    ADC_INDPENDENT_NUM_0                      = 0x0,  /**< Independent mode*/
...
typedef enum
{
    ADC_SW_REGU_TRIGER                          = 0x0 ,   /**< ADC Start a conversion on  regular  channel */
...
#define HAL_ADC_ERROR_DMA         ((uint32_t)0x02)   /*!< DMA transfer error   */
  ...
  typedef struct
  {
    ...
    uint32_t EOCSelection;          /*!< Specifies whether the EOC flag is set
     ...
     uint32_t dma_en;
    uint32_t DMAContinuousRequests; /*!< Specifies whether the DMA requests is performed in repeat(Continuous) or in Single mode.
                                         This parameter can be set to ENABLE or DISABLE. */
  ...
  typedef struct
  {
    ADC_Device_T                   *Instance;                   /*!< Register base address */
