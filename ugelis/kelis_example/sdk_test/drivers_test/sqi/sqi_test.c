...
#define FLASH_TEST_ERASE_SIZE    0x100000
#define FLASH_TEST_REGION_OFFSET 0x0
#define FLASH_SECTOR_SIZE        (4096)
#define FLASH_TOTALSIZE_LEN      (0x100000)
...
int sqi_flash_write_read_demo(void)
{
    ...
    TC_PRINT("********SFlash CPU Read and Write ZTest Begin*******\n");
    ret  = HAL_SQI_Init(&gSQIHandle, pSQIDev, &gSQIConfig);
    if (ret != RET_OK)
    {
        TC_PRINT("********SFlash Init Error*******\n");
...
int sqi_flash_writeall_read_demo(void)
{
    ...
    TC_PRINT("********SFlash Write All ZTest Begin*******\n");
    ret  = HAL_SQI_Init(&gSQIHandle, pSQIDev, &gSQIConfig);
    if (ret != RET_OK)
    {
        TC_PRINT("********SFlash Init Error*******\n");
...
int sqi_flash_dma_write_read_demo(void)
{
    ...
    uint32_t    value_t;
    RET_CODE    ret;
    uint32_t    sector_offset, sector_count;
    sector_offset = FLASH_TEST_OFFSET / FLASH_SECTOR_SIZE;
    sector_count = (FLASH_TOTALSIZE_LEN - FLASH_TEST_OFFSET) / FLASH_SECTOR_SIZE;
    memset(buff_to_flash, 0x00, FLASH_SECTOR_SIZE);
    ...
    TC_PRINT("********SFlash DMA Read and Write ZTest Begin*******\n");
    ...
    TC_PRINT("The Flash DMA Write & Read Data all OK !\n");
