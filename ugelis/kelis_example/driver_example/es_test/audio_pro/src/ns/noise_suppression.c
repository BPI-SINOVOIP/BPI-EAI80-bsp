...
#define tanhf(x) ((exp(x) - exp(-x))/(exp(x) + exp(-x)))

double tanhf_x(double x)
{
    double val = 0;
                ...
                float *magn)
{
    ...
    for (i = 1; i < magnitude_length - 1; ++i)
    {
        ...
        //printf("IFFT step0 real[%d]=%f, imag[%d]=%f, magn[%d] = %f\r\n", i, real[i], i, imag[i], i, magn[i]); // OK
                                       ...
                                       float *theFilter)
{
    ...
    for (i = 0; i < self->magnLen; i++)
    {
        ...
        //printf("IFFT xxx theFilter[%d] = %f, snrPrior=%f, overdrive=%f\r\n", i, theFilter[i], snrPrior, self->overdrive);
    }  // End of loop over frequencies.
}
...
float g_winData[ANAL_BLOCKL_MAX];
float g_magn[HALF_ANAL_BLOCKL], noise[HALF_ANAL_BLOCKL];
float snrLocPost[HALF_ANAL_BLOCKL], snrLocPrior[HALF_ANAL_BLOCKL];
float g_real[ANAL_BLOCKL_MAX], g_imag[HALF_ANAL_BLOCKL];
...
void WebRtcNs_AnalyzeCore(NoiseSuppressionC *self, const float *speechFrame)
{
    ...
    Windowing(self->window, self->analyzeBuf, self->anaLen, g_winData);
    energy = Energy(g_winData, self->anaLen);
    if (energy == 0.0)
    {
    ...
    FFT(self, g_winData, self->anaLen, self->magnLen, g_real, g_imag, g_magn);

    for (i = 0; i < self->magnLen; i++)
    {
        signalEnergy += g_real[i] * g_real[i] + g_imag[i] * g_imag[i];
        sumMagn += g_magn[i];
    ...
    NoiseEstimation(self, g_magn, noise);
    ...
    ComputeSnr(self, g_magn, noise, snrLocPrior, snrLocPost);

    FeatureUpdate(self, g_magn, updateParsFlag);
    SpeechNoiseProb(self, self->speechProb, snrLocPrior, snrLocPost);
    UpdateNoiseEstimate(self, g_magn, noise);

    // Keep track of noise spectrum for next frame.
    memcpy(self->noise, noise, sizeof(*noise) * self->magnLen);
    memcpy(self->magnPrevAnalyze, g_magn, sizeof(*g_magn) * self->magnLen);
}
                          ...
                          float *const *outFrame)
{
    ...
    //printf("energy1=%f, self->anaLen=%d\n", energy1, self->anaLen);
    if (energy1 == 0.0)
    {
    ...
    for (i = 0; i < self->magnLen; i++)
    {
        ...
        //printf("IFFT xxx real[%d]=%f, imag[%d]=%f, self->smooth[%d] = %f\r\n", i, real[i], i, imag[i], i, self->smooth[i]);
    }
    ...
    for (i = 0; i < self->anaLen; i++)
    {
        ...
        //printf("winData[%d] = %f, factor=%d \n", i, winData[i], factor);
    }
    // Read out fully processed segment.
    for (i = self->windShift; i < self->blockLen + self->windShift; i++)
    {
        fout[i - self->windShift] = self->syntBuf[i];
        // printf("fout[%d] = %f, self->windShift=%d \n", i, fout[i], self->windShift);
    }
    // Update synthesis buffer.
    UpdateBuffer(NULL, self->blockLen, self->anaLen, self->syntBuf);
