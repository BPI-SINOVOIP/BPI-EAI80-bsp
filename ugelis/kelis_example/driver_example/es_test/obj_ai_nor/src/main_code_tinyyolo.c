...
typedef unsigned short u16;
    ...
    unsigned char CMD_BASE[32 * 1024] __in_section_unique(mem_ai_data);

    __aligned(32) unsigned char WEI_BASE [5 * 1024 * 1024]__in_section_unique(mem_ai_data);  /*more than 4 MB are needed for YOLO*/

    #ifdef BODY_DETECTION_DEMO
        __aligned(32) unsigned char WEI_BASE_BODY[5 * 1024 * 1024]__in_section_unique(mem_ai_data); /*more than 4 MB are needed for YOLO*/
    #endif

    #ifdef GESTURE_DETECTION_DEMO
        __aligned(32) unsigned char WEI_BASE_GEST[5 * 1024 * 1024]__in_section_unique(mem_ai_data); /*more than 4 MB are needed for YOLO*/
    #endif

    #ifdef BODY_DETECTION_DEMO
        unsigned char RecvBufferBody[153600] __in_section_unique(mem_ai_data); //TEST_BUFFER_SIZE
    #endif

    #ifdef GESTURE_DETECTION_DEMO
        unsigned char RecvBufferGest[153600] __in_section_unique(mem_ai_data);//TEST_BUFFER_SIZE
    #endif

    unsigned char CMD_BASE_BODY[32 * 1024] __in_section_unique(mem_ai_data);
    unsigned char CMD_BASE_GEST[32 * 1024] __in_section_unique(mem_ai_data);

    #ifdef  TINY_YOLO_FOR_OBJT
        __aligned(32) unsigned char WEI_BASE[22 * 1024 * 1024]__in_section_unique(mem_ai_data);  /*more than 4 MB are needed for YOLO*/
    #endif

    unsigned char UNREG_DUMP_BUFF[IMG_DUMP_PIXELS]__in_section_unique(mem_ai_data);
    unsigned char REG_DUMP_BUFF[IMG_DUMP_PIXELS]__in_section_unique(mem_ai_data);

    unsigned char MAGFLG_FOR_MODELLOADED[4]__in_section_unique(mem_ai_data);
#else
    unsigned short *inputImage  = (uint16_t *)INPUT_IMAGE_START_ADDR;
    unsigned short *OutputImage = (uint16_t *)OUTPUT_IMAGE_START_ADDR;

    unsigned char  *CMD_BASE = (uint8_t *)CMD_BASE_START_ADDR;
    unsigned char  *CMD_BASE_BODY = (uint8_t *)CMD_BASE_BODY_START_ADDR;
    unsigned char  *CMD_BASE_GEST = (uint8_t *)CMD_BASE_GEST_START_ADDR;

    unsigned char  *WEI_BASE = (uint8_t *)WEI_BASE_START_ADDR;
    unsigned char  *WEI_BASE_BODY = (uint8_t *)WEI_BASE_BODY_START_ADDR;
    unsigned char  *WEI_BASE_GEST = (uint8_t *)WEI_BASE_GEST_START_ADDR;

    unsigned char  *RecvBuffer = (uint8_t *)RECV_BUF_START_ADDR;
    unsigned char  *RecvBufferBody = (uint8_t *)RECV_BUF_BODY_START_ADDR;
    unsigned char  *RecvBufferGest = (uint8_t *)RECV_BUF_GEST_START_ADDR;

    unsigned char  *UNREG_DUMP_BUFF = (uint8_t *)UNREG_DUMP_BUFF_START_ADDR;
    unsigned char  *REG_DUMP_BUFF   = (uint8_t *)REG_DUMP_BUFF_START_ADDR;
#endif
...
void PostYolo(void)
{
    ...
    box = malloc_ext(0, out_width * out_height * box_num * box_channel * sizeof(double)); //malloc(0, out_width*out_height*box_num*box_channel * sizeof(double));//calloc(out_width*out_height*box_num*box_channel,sizeof(double));
    box_class_probs = malloc_ext(0, out_height * out_width * box_num * class_channel * sizeof(double)); //malloc(0, out_height*out_width*box_num*class_channel * sizeof(double));//calloc(out_height*out_width*box_num*class_channel,sizeof(double));
...
{
    int channel = 0, r = 0, c = 0;
    for (channel = 0; channel < NumChannel; ++channel)
    {
        for (r = 0; r < NumRow; ++r)
        {
            for (c = 0; c < NumColPad; ++c)
            {
                if (c < NumCol)
                {
                    inputImage[channel * NumRow * NumColPad + r * NumColPad + c] = OutputImage[channel * NumRow * NumCol + r * NumCol + c];
                }
                else
                {
                    inputImage[channel * NumRow * NumColPad + r * NumColPad + c] = 0;
                }
            }
        }
    }
