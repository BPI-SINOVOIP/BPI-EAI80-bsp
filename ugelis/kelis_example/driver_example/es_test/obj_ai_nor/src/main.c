...
static uint8_t usb_initialed = 0;
    ...
    *(volatile uint32_t *)(0x4000009c) |= 2 << 16;
    ...
    while (1)
    {
        ...
        //      if (camvedio_read_done == 1)
        {
            ...
            JPG_enc_buf[MJPEG_SIZE - 2] = 0xFF;
            JPG_enc_buf[MJPEG_SIZE - 1] = 0xD9;

            for (i = 0; i < MJPEG_SIZE; i++)
            {
                g_UsbDeviceVideoMjpegData[i + mjpeg_index * MJPEG_SIZE] = JPG_enc_buf[i];
            }

            mjpeg_index ++;
            mjpeg_index = mjpeg_index % MJPEG_COUNT_FOR_BUFFER;

            /*ensure that there must be mjpeg data in the buffer !!!*/
            if (mjpeg_index == MJPEG_COUNT_FOR_BUFFER - 1)
            {
                if (usb_initialed != 1)
                {
                    /*usb vertual camera init*/
