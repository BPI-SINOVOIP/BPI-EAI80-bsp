...
int _impl__ugelis_read(char *buf, int nbytes)
{
	int i = 0;
...
Z_SYSCALL_HANDLER(_ugelis_read, buf, nbytes)
{
	Z_OOPS(Z_SYSCALL_MEMORY_WRITE(buf, nbytes));
	return _impl__ugelis_read((char *)buf, nbytes);
}
#endif

int _impl__ugelis_write(char *buf, int nbytes)
{
	int i;
...
#ifdef CONFIG_USERSPACE
Z_SYSCALL_HANDLER(_ugelis_write, buf, nbytes)
{
	Z_OOPS(Z_SYSCALL_MEMORY_READ(buf, nbytes));
	return _impl__ugelis_write((char *)buf, nbytes);
}
#endif
...
int _read(int fd, char *buf, int nbytes)
{
	...
	return _ugelis_read(buf, nbytes);
}
...
int _write(int fd, char *buf, int nbytes)
{
	...
	return _ugelis_write(buf, nbytes);
}
