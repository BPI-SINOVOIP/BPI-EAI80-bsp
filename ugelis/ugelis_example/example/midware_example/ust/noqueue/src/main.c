...
static int ust_get_element(UST_Element_T **pEltSrc, UST_Element_T **pEltMiddle, UST_Element_T **pEltsink);
static int ust_get_pad(UST_Pad_T **pad_table, UST_Element_T *eltSrc,
                       UST_Element_T *eltMiddle, UST_Element_T *eltsink);
static void ust_add_element_to_pipeline(UST_PipeLine_T *line, UST_Element_T *eltSrc,
                                        UST_Element_T *eltMiddle, UST_Element_T *eltsink);
...
static int ust_get_element(UST_Element_T **pEltSrc, UST_Element_T **pEltMiddle, UST_Element_T **pEltsink)
{
    ...
    *pEltsink = UST_GetElement(ELEMENT_SRINK);
    ...
    ust_demo_pr_debug("<--get element: src_element[name:%s]:%p  mid_element[name:%s]:%p sink_element[name:%s]:%p----->\n",
                      (*pEltSrc)->name,
                      *pEltSrc,
                      (*pEltMiddle)->name,
                      *pEltMiddle,
                       ...
                       UST_Element_T *eltMiddle, UST_Element_T *eltsink)
{
    ...
    ust_demo_pr_debug("[%s] srcElement:srcpad=%p midElement:sinkpad=%p midElement:srcpad=%p sinkElement:sinkpad=%p\n",
                      __FUNCTION__,
                      pad_table[ELEMENT_SRC_PAD_SRC_INDEX],
                      pad_table[ELEMENT_MIDDLE_PAD_SINK_INDEX], pad_table[ELEMENT_MIDDLE_PAD_SRC_INDEX],
...
static void ust_pad_link(UST_Pad_T **pad_table)
{
    UST_Pad_T *srcPad               = pad_table[ELEMENT_SRC_PAD_SRC_INDEX];
    ...
    UST_PadConnect(srcPad, eltMiddleSinkPad);
...
int main(void)
{
     ...
     *pileline-->eltSrc-->eltMiddle--->srink
     */
    ret = ust_get_element(&eltSrc, &eltMiddle, &eltsink);
    if (ret)
    {
        ust_demo_pr_err("[%s]:Err!!!!: ust_get_element NULL\n", __FUNCTION__);
        goto out;
    }
    ust_add_element_to_pipeline(pipeLine, eltSrc, eltMiddle, eltsink);

    /**
     * connet pipeLine pad sequence
     * srcPad<--link-->eltMiddleSinkPad   eltMiddle1SrcPad<---link---->eltsinkPad
     */
    ret = ust_get_pad(pad_table, eltSrc, eltMiddle, eltsink);
    if (ret)
    {
