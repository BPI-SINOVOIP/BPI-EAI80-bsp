...
static struct ugelis_smp_transport smp_uart_transport;
...
static void smp_uart_process_frag(struct uart_mcumgr_rx_buf *rx_buf)
{
	 ...
	 */
	if (nb != NULL) {
		ugelis_smp_rx_req(&smp_uart_transport, nb);
	}
}
...
static int smp_uart_tx_pkt(struct ugelis_smp_transport *zst,
			   struct net_buf *nb)
{
	int rc;
...
static int smp_uart_init(struct device *dev)
{
	...
	ugelis_smp_transport_init(&smp_uart_transport, smp_uart_tx_pkt,
