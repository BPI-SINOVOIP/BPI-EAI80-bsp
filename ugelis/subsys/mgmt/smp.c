...
static mgmt_alloc_rsp_fn ugelis_smp_alloc_rsp;
static mgmt_trim_front_fn ugelis_smp_trim_front;
static mgmt_reset_buf_fn ugelis_smp_reset_buf;
static mgmt_write_at_fn ugelis_smp_write_at;
static mgmt_init_reader_fn ugelis_smp_init_reader;
static mgmt_init_writer_fn ugelis_smp_init_writer;
static mgmt_free_buf_fn ugelis_smp_free_buf;
static smp_tx_rsp_fn ugelis_smp_tx_rsp;

static const struct mgmt_streamer_cfg ugelis_smp_cbor_cfg = {
	.alloc_rsp = ugelis_smp_alloc_rsp,
	.trim_front = ugelis_smp_trim_front,
	.reset_buf = ugelis_smp_reset_buf,
	.write_at = ugelis_smp_write_at,
	.init_reader = ugelis_smp_init_reader,
	.init_writer = ugelis_smp_init_writer,
	.free_buf = ugelis_smp_free_buf,
};

void *
ugelis_smp_alloc_rsp(const void *req, void *arg)
{
	const struct net_buf_pool *pool;
	const struct net_buf *req_nb;
	struct net_buf *rsp_nb;
	struct ugelis_smp_transport *zst = arg;
...
static void
ugelis_smp_trim_front(void *buf, size_t len, void *arg)
{
	struct net_buf *nb;
 ...
 *     // [...]
 *     while (rsp != NULL) {
 *         frag = ugelis_smp_split_frag(&rsp, zst, get_mtu());
 *         if (frag == NULL) {
 *             net_buf_unref(nb);
 *             return SYS_ENOMEM;
 *         }
 ...
 * @param arg                   The ugelis SMP transport pointer.
...
ugelis_smp_split_frag(struct net_buf **nb, void *arg, u16_t mtu)
{
	struct net_buf *frag;
	struct net_buf *src;
...
static void
ugelis_smp_reset_buf(void *buf, void *arg)
{
	net_buf_reset(buf);
}

static int
ugelis_smp_write_at(struct cbor_encoder_writer *writer, size_t offset,
...
static int
ugelis_smp_tx_rsp(struct smp_streamer *ns, void *rsp, void *arg)
{
	struct ugelis_smp_transport *zst;
	struct net_buf *frag;
	struct net_buf *nb;
	u16_t mtu;
	int rc;
	...
	i = 0;
	while (nb != NULL) {
		frag = ugelis_smp_split_frag(&nb, zst, mtu);
		if (frag == NULL) {
...
static void
ugelis_smp_free_buf(void *buf, void *arg)
{
	struct ugelis_smp_transport *zst = arg;
...
static int
ugelis_smp_init_reader(struct cbor_decoder_reader *reader, void *buf,
		       void *arg)
{
...
static int
ugelis_smp_init_writer(struct cbor_encoder_writer *writer, void *buf,
		       void *arg)
{
	struct cbor_nb_writer *czw;
 ...
 */
static int
ugelis_smp_process_packet(struct ugelis_smp_transport *zst,
			  struct net_buf *nb)
{
	...
	streamer = (struct smp_streamer) {
		.mgmt_stmr = {
			.cfg = &ugelis_smp_cbor_cfg,
			.reader = &reader.r,
			.writer = &writer.enc,
			.cb_arg = zst,
		},
		.tx_rsp_cb = ugelis_smp_tx_rsp,
	};
 ...
 */
static void
ugelis_smp_handle_reqs(struct k_work *work)
{
	struct ugelis_smp_transport *zst;
	struct net_buf *nb;
	...
	while ((nb = k_fifo_get(&zst->zst_fifo, K_NO_WAIT)) != NULL) {
		ugelis_smp_process_packet(zst, nb);
	}
}

void
ugelis_smp_transport_init(struct ugelis_smp_transport *zst,
			  ugelis_smp_transport_out_fn *output_func,
			  ugelis_smp_transport_get_mtu_fn *get_mtu_func,
			  ugelis_smp_transport_ud_copy_fn *ud_copy_func,
			  ugelis_smp_transport_ud_free_fn *ud_free_func)
{
	*zst = (struct ugelis_smp_transport) {
	...
	k_work_init(&zst->zst_work, ugelis_smp_handle_reqs);
	k_fifo_init(&zst->zst_fifo);
}

void
ugelis_smp_rx_req(struct ugelis_smp_transport *zst, struct net_buf *nb)
{
