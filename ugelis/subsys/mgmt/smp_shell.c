...
static struct ugelis_smp_transport smp_shell_transport;
...
static int smp_shell_rx_line(const char *line, void *arg)
{
	...
	if (nb != NULL) {
		ugelis_smp_rx_req(&smp_shell_transport, nb);
	}
...
static int smp_shell_tx_pkt(struct ugelis_smp_transport *zst,
			    struct net_buf *nb)
{
	int rc;
...
static int smp_shell_init(struct device *dev)
{
	...
	ugelis_smp_transport_init(&smp_shell_transport, smp_shell_tx_pkt,
