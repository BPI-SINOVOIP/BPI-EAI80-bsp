...
static struct ugelis_smp_transport smp_bt_transport;
				...
				u8_t flags)
{
	...
	ugelis_smp_rx_req(&smp_bt_transport, nb);
 ...
 */
static int smp_bt_tx_pkt(struct ugelis_smp_transport *zst, struct net_buf *nb)
{
	struct bt_conn *conn;
	int rc;
...
static int smp_bt_init(struct device *dev)
{
	...
	ugelis_smp_transport_init(&smp_bt_transport, smp_bt_tx_pkt,
