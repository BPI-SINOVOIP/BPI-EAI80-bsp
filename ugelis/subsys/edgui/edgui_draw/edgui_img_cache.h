/**
 * @file edgui_img_cache.h
 *
 */

#ifndef EDGUI_IMG_CACHE_H
#define EDGUI_IMG_CACHE_H
...
typedef struct
{
    edgui_img_decoder_dsc_t dec_dsc; /**< Image information */

    /** Count the cache entries's life. Add `time_tio_open` to `life` when the entry is used.
     * Decrement all lifes by one every in every ::edgui_img_cache_open.
     * If life == 0 the entry can be reused */
    int32_t life;
} edgui_img_cache_entry_t;
 ...
 */
void edgui_img_cache_set_size(uint16_t new_slot_num);
