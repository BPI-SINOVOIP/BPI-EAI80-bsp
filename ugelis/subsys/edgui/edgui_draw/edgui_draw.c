...
{
    if(size <= draw_buf_size) return EDGUI_GC_ROOT(_edgui_draw_buf);

    EDGUI_LOG_TRACE("edgui_draw_get_buf: allocate");

    draw_buf_size = size;

    if(EDGUI_GC_ROOT(_edgui_draw_buf) == NULL) {
        EDGUI_GC_ROOT(_edgui_draw_buf) = edgui_mem_alloc(size);
        edgui_mem_assert(EDGUI_GC_ROOT(_edgui_draw_buf));
        return EDGUI_GC_ROOT(_edgui_draw_buf);
    }

    EDGUI_GC_ROOT(_edgui_draw_buf) = edgui_mem_realloc(EDGUI_GC_ROOT(_edgui_draw_buf), size);
    edgui_mem_assert(EDGUI_GC_ROOT(_edgui_draw_buf));
    return EDGUI_GC_ROOT(_edgui_draw_buf);
}
...
{
    if(EDGUI_GC_ROOT(_edgui_draw_buf)) {
        edgui_mem_free(EDGUI_GC_ROOT(_edgui_draw_buf));
        EDGUI_GC_ROOT(_edgui_draw_buf) = NULL;
        draw_buf_size = 0;
    }
}
...
edgui_opa_t edgui_draw_aa_get_opa(edgui_coord_t seg, edgui_coord_t px_id, edgui_opa_t base_opa)
{
    ...
    static const edgui_opa_t seg1[1] = {128};
    static const edgui_opa_t seg2[2] = {64, 192};
    static const edgui_opa_t seg3[3] = {42, 128, 212};
    static const edgui_opa_t seg4[4] = {32, 96, 159, 223};
    static const edgui_opa_t seg5[5] = {26, 76, 128, 178, 230};
    static const edgui_opa_t seg6[6] = {21, 64, 106, 148, 191, 234};
    static const edgui_opa_t seg7[7] = {18, 55, 91, 128, 164, 200, 237};
    static const edgui_opa_t seg8[8] = {16, 48, 80, 112, 143, 175, 207, 239};

    static const edgui_opa_t * seg_map[] = {seg1, seg2, seg3, seg4, seg5, seg6, seg7, seg8};

    if(seg == 0)
        return EDGUI_OPA_TRANSP;
    else if(seg < 8)
                        ...
                        edgui_opa_t opa)
{
    ...
    edgui_coord_t i;
    for(i = 0; i < length; i++) {
        edgui_opa_t px_opa = edgui_draw_aa_get_opa(length, i, opa);
        if(aa_inv) px_opa = opa - px_opa;
        edgui_draw_px(x, y + i, mask, color, px_opa);
    }
}
                        ...
                        edgui_opa_t opa)
{
    ...
    for(i = 0; i < length; i++) {
        edgui_opa_t px_opa = edgui_draw_aa_get_opa(length, i, opa);
        if(aa_inv) px_opa = opa - px_opa;
        edgui_draw_px(x + i, y, mask, color, px_opa);
    }
}
