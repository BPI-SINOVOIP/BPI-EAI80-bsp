 ...
 *********************/
#define DEF_RADIUS 4
#define DEF_SHADOW_COLOR edgui_color_hex3(0xaaa)
...
{
    static edgui_style_t bg, panel, scr;
    ...
    bg.body.main_color = edgui_color_hex(0xf0f0f0);
    ...
    panel.body.main_color     = EDGUI_COLOR_WHITE;
    panel.body.grad_color     = EDGUI_COLOR_WHITE;
    panel.body.border.width   = 1;
    panel.body.border.color   = edgui_color_hex3(0xbbb);
    panel.body.border.opa     = EDGUI_OPA_COVER;
    panel.body.shadow.color   = DEF_SHADOW_COLOR;
    panel.body.shadow.type    = EDGUI_SHADOW_BOTTOM;
    panel.body.shadow.width   = 4;
    panel.body.padding.left   = EDGUI_DPI / 8;
    panel.body.padding.right  = EDGUI_DPI / 8;
    panel.body.padding.top    = EDGUI_DPI / 8;
    panel.body.padding.bottom = EDGUI_DPI / 8;
    panel.body.padding.inner  = EDGUI_DPI / 12;
    panel.text.color          = edgui_color_hex3(0x333);
    panel.image.color         = edgui_color_hex3(0x333);

    edgui_style_copy(&sb, &def);
    sb.body.main_color     = EDGUI_COLOR_BLACK;
    sb.body.grad_color     = EDGUI_COLOR_BLACK;
    sb.body.opa            = EDGUI_OPA_40;
    sb.body.padding.right  = EDGUI_DPI / 25;
    sb.body.padding.bottom = EDGUI_DPI / 25;
...
static void btn_init(void)
{
    ...
    rel.body.main_color     = edgui_color_hsv_to_rgb(_hue, 90, 70);
    rel.body.grad_color     = rel.body.main_color;
    rel.body.radius         = DEF_RADIUS;
    rel.body.padding.left   = EDGUI_DPI / 6;
    rel.body.padding.right  = EDGUI_DPI / 6;
    rel.body.padding.top    = EDGUI_DPI / 8;
    rel.body.padding.bottom = EDGUI_DPI / 8;
    rel.body.padding.inner  = EDGUI_DPI / 10;
    rel.body.shadow.color   = DEF_SHADOW_COLOR;
    rel.body.shadow.type    = EDGUI_SHADOW_BOTTOM;
    rel.body.shadow.width   = 6;
    rel.text.color          = edgui_color_hsv_to_rgb(_hue, 5, 95);
    rel.image.color         = edgui_color_hsv_to_rgb(_hue, 5, 95);

    edgui_style_copy(&pr, &rel);
    pr.body.main_color   = edgui_color_hsv_to_rgb(_hue, 90, 60);
    pr.body.grad_color   = pr.body.main_color;
    pr.body.shadow.width = 4;

    edgui_style_copy(&tgl_rel, &rel);
    tgl_rel.body.main_color   = edgui_color_hsv_to_rgb(_hue, 95, 50);
    tgl_rel.body.grad_color   = tgl_rel.body.main_color;
    tgl_rel.body.shadow.width = 4;

    edgui_style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color   = edgui_color_hsv_to_rgb(_hue, 95, 40);
    tgl_pr.body.grad_color   = tgl_pr.body.main_color;
    tgl_pr.body.shadow.width = 2;

    edgui_style_copy(&ina, &rel);
    ina.body.main_color   = edgui_color_hex3(0xccc);
    ina.body.grad_color   = ina.body.main_color;
    ina.body.shadow.width = 0;
    ina.text.color        = edgui_color_hsv_to_rgb(_hue, 95, 5);
    ina.image.color       = edgui_color_hsv_to_rgb(_hue, 95, 5);
...
static void label_init(void)
{
    ...
    prim.text.color = edgui_color_hsv_to_rgb(_hue, 80, 10);

    edgui_style_copy(&sec, &prim);
    sec.text.color = edgui_color_hsv_to_rgb(_hue, 80, 75);

    edgui_style_copy(&hint, &prim);
    hint.text.color = edgui_color_hsv_to_rgb(_hue, 40, 90);
...
static void led_init(void)
{
    ...
    led.body.grad_color   = edgui_color_hsv_to_rgb(_hue, 100, 100);
...
static void bar_init(void)
{
    ...
    bar_bg.body.main_color     = edgui_color_hsv_to_rgb(_hue, 15, 95);
    ...
    edgui_style_copy(&bar_indic, &bar_bg);
    bar_indic.body.main_color     = edgui_color_hsv_to_rgb(_hue, 85, 70);
...
{
#if EDGUI_USE_SW != 0
    static edgui_style_t sw_bg, sw_indic, sw_knob_off, sw_knob_on;
    edgui_style_copy(&sw_bg, theme.style.slider.bg);
    sw_bg.body.radius = EDGUI_RADIUS_CIRCLE;

    edgui_style_copy(&sw_indic, theme.style.slider.bg);
    sw_indic.body.radius = EDGUI_RADIUS_CIRCLE;

    edgui_style_copy(&sw_knob_on, theme.style.slider.knob);
    sw_knob_on.body.shadow.width = 3;
    sw_knob_on.body.shadow.type  = EDGUI_SHADOW_BOTTOM;
    sw_knob_on.body.shadow.color = DEF_SHADOW_COLOR;

    edgui_style_copy(&sw_knob_off, &sw_knob_on);
    sw_knob_off.body.main_color   = edgui_color_hex(0xfafafa);
    sw_knob_off.body.grad_color   = sw_knob_off.body.main_color;
    sw_knob_off.body.border.width = 1;
    sw_knob_off.body.border.color = edgui_color_hex3(0x999);
    sw_knob_off.body.border.opa   = EDGUI_OPA_COVER;
...
static void lmeter_init(void)
{
    ...
    lmeter.body.main_color   = edgui_color_hsv_to_rgb(_hue, 75, 90);
    lmeter.body.grad_color   = lmeter.body.main_color;
    lmeter.body.padding.left = EDGUI_DPI / 10; /*Scale line length*/
    lmeter.line.color        = edgui_color_hex3(0x999);
    lmeter.line.width        = 2;
...
static void gauge_init(void)
{
    ...
    gauge.body.main_color    = edgui_color_hsv_to_rgb(_hue, 10, 60);
    gauge.body.grad_color    = gauge.body.main_color;
    gauge.body.padding.left  = EDGUI_DPI / 16; /*Scale line length*/
    gauge.body.padding.inner = EDGUI_DPI / 8;
    gauge.body.border.color  = edgui_color_hex3(0x999);
    gauge.text.color         = edgui_color_hex3(0x333);
    gauge.line.width         = 3;
    gauge.line.color         = edgui_color_hsv_to_rgb(_hue, 95, 70);
...
static void arc_init(void)
{
    ...
    arc.line.width = 10;
    arc.line.color = edgui_color_hsv_to_rgb(_hue, 90, 90);

    /*For prelaoder*/
    arc.body.border.width   = 10;
    arc.body.border.color   = edgui_color_hsv_to_rgb(_hue, 30, 90);
...
static void calendar_init(void)
{
    ...
    high_days.text.color = edgui_color_hsv_to_rgb(_hue, 80, 90);

    static edgui_style_t week_box;
    edgui_style_copy(&week_box, &def);
    week_box.body.main_color     = edgui_color_hsv_to_rgb(_hue, 40, 100);
    week_box.body.grad_color     = edgui_color_hsv_to_rgb(_hue, 40, 100);
    ...
    week_box.body.border.part    = EDGUI_BORDER_LEFT | EDGUI_BORDER_RIGHT;
...
#if EDGUI_USE_CB != 0
    static edgui_style_t rel, pr, tgl_rel, tgl_pr, ina;
    edgui_style_copy(&rel, theme.style.panel);
    rel.body.shadow.type  = EDGUI_SHADOW_BOTTOM;
    rel.body.shadow.width = 3;

    edgui_style_copy(&pr, &rel);
    pr.body.main_color   = edgui_color_hex3(0xccc);
    pr.body.grad_color   = pr.body.main_color;
    pr.body.shadow.width = 0;

    edgui_style_copy(&tgl_rel, &rel);
    tgl_rel.body.main_color   = edgui_color_hsv_to_rgb(_hue, 75, 85);
    tgl_rel.body.grad_color   = tgl_rel.body.main_color;
    tgl_rel.body.shadow.type  = EDGUI_SHADOW_FULL;
    tgl_rel.body.shadow.width = 0;

    edgui_style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color   = edgui_color_hsv_to_rgb(_hue, 75, 65);
    tgl_pr.body.grad_color   = tgl_pr.body.main_color;
...
static void btnm_init(void)
{
    ...
    bg.text.color          = edgui_color_hex3(0x555);

    edgui_style_copy(&rel, theme.style.panel);
    rel.body.border.part  = EDGUI_BORDER_FULL | EDGUI_BORDER_INTERNAL;
    rel.body.border.width = 1;
    rel.body.border.color = edgui_color_hex3(0xbbb);
    rel.body.opa          = EDGUI_OPA_TRANSP;
    rel.body.shadow.width = 0;

    edgui_style_copy(&pr, &rel);
    pr.glass             = 0;
    pr.body.main_color   = edgui_color_hex3(0xddd);
    pr.body.grad_color   = pr.body.main_color;
    pr.body.border.width = 0;
    pr.body.opa          = EDGUI_OPA_COVER;

    edgui_style_copy(&tgl_rel, &pr);
    tgl_rel.body.main_color = edgui_color_hsv_to_rgb(_hue, 90, 70);
    tgl_rel.body.grad_color = tgl_rel.body.main_color;
    tgl_rel.text.color      = edgui_color_hsv_to_rgb(_hue, 5, 95);

    edgui_style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color   = edgui_color_hsv_to_rgb(_hue, 95, 65);
    tgl_pr.body.grad_color   = tgl_pr.body.main_color;
...
static void kb_init(void)
{
    ...
    edgui_style_copy(&rel, &edgui_style_transp);
    rel.text.font = _font;
...
{
#if EDGUI_USE_MBOX
    static edgui_style_t pr, rel;

    edgui_style_copy(&rel, &edgui_style_transp);
    rel.glass      = 0;
    rel.text.font  = _font;
    rel.text.color = edgui_color_hsv_to_rgb(_hue, 85, 75);

    edgui_style_copy(&pr, theme.style.btnm.btn.pr);
    pr.text.color = edgui_color_hsv_to_rgb(_hue, 85, 60);
...
static void ta_init(void)
{
    ...
    edgui_style_copy(&oneline, &def);
    oneline.body.opa          = EDGUI_OPA_TRANSP;
    oneline.body.radius       = 0;
    oneline.body.border.part  = EDGUI_BORDER_BOTTOM;
    oneline.body.border.width = 3;
    oneline.body.border.color = edgui_color_hex3(0x333);
    oneline.body.border.opa   = EDGUI_OPA_COVER;
    oneline.text.color        = edgui_color_hex3(0x333);
...
static void list_init(void)
{
    ...
    static edgui_style_t list_bg, rel, pr, tgl_rel, tgl_pr, ina;

    edgui_style_copy(&list_bg, theme.style.panel);
    ...
    rel.body.padding.top    = EDGUI_DPI / 6;
    rel.body.padding.bottom = EDGUI_DPI / 6;
    rel.body.radius         = 10;
    rel.body.border.color   = edgui_color_hex3(0xbbb);
    rel.body.border.width   = 1;
    rel.body.border.part    = EDGUI_BORDER_BOTTOM;
    ...
    edgui_style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color   = edgui_color_hsv_to_rgb(_hue, 90, 60);
    tgl_pr.body.grad_color   = tgl_pr.body.main_color;
    tgl_pr.body.border.width = 0;
...
static void ddlist_init(void)
{
    ...
    bg.body.padding.top    = EDGUI_DPI / 6;
    bg.body.padding.bottom = EDGUI_DPI / 6;
    bg.text.line_space     = EDGUI_DPI / 8;

    edgui_style_copy(&sel, &bg);
    sel.body.main_color   = edgui_color_hsv_to_rgb(_hue, 90, 70);
    sel.body.grad_color   = sel.body.main_color;
    sel.body.border.width = 0;
    sel.body.shadow.width = 0;
    sel.text.color        = edgui_color_hsv_to_rgb(_hue, 5, 95);
...
static void roller_init(void)
{
    ...
    edgui_style_copy(&roller_bg, &edgui_style_transp);
    roller_bg.body.padding.left   = EDGUI_DPI / 6;
    roller_bg.body.padding.right  = EDGUI_DPI / 6;
    roller_bg.body.padding.top    = EDGUI_DPI / 6;
    roller_bg.body.padding.bottom = EDGUI_DPI / 6;
    roller_bg.text.line_space     = EDGUI_DPI / 8;
    roller_bg.text.font           = _font;
    roller_bg.glass               = 0;

    edgui_style_copy(&roller_sel, &roller_bg);
    roller_sel.text.color = edgui_color_hsv_to_rgb(_hue, 90, 70);
...
static void tabview_init(void)
{
    ...
    btn_bg.text.color          = edgui_color_hex3(0x333);
    ...
    pr.body.main_color   = edgui_color_hex3(0xbbb);
    pr.body.grad_color   = pr.body.main_color;
    pr.body.border.width = 0;
    pr.body.opa          = EDGUI_OPA_COVER;
    pr.body.radius       = 0;
    ...
    pr.body.border.part  = EDGUI_BORDER_BOTTOM;
    pr.body.border.opa   = EDGUI_OPA_COVER;
    pr.text.color        = edgui_color_hex3(0x111);

    edgui_style_copy(&tgl_rel, &edgui_style_transp);
    tgl_rel.glass      = 0;
    tgl_rel.text.font  = _font;
    tgl_rel.text.color = edgui_color_hsv_to_rgb(_hue, 90, 70);

    edgui_style_copy(&tgl_pr, &def);
    tgl_pr.body.main_color   = edgui_color_hsv_to_rgb(_hue, 15, 85);
    tgl_pr.body.grad_color   = tgl_pr.body.main_color;
    tgl_pr.body.border.width = 0;
    tgl_pr.body.opa          = EDGUI_OPA_COVER;
    tgl_pr.body.radius       = 0;
    tgl_pr.text.color        = edgui_color_hsv_to_rgb(_hue, 90, 60);
...
static void win_init(void)
{
    ...
    header.text.color          = edgui_color_hex3(0x333);
    header.image.color         = edgui_color_hex3(0x333);
    ...
    pr.body.radius       = 0;
    pr.text.color        = edgui_color_hex3(0x111);
    pr.image.color       = edgui_color_hex3(0x111);
...
static void style_mod(edgui_group_t * group, edgui_style_t * style)
{
    ...
    style->body.border.color = edgui_color_hsv_to_rgb(hue2, 90, 70);

    /*If not empty or has border then emphasis the border*/
    if(style->body.opa != EDGUI_OPA_TRANSP || style->body.border.width != 0) style->body.border.width = EDGUI_DPI / 30;

    style->body.main_color   = edgui_color_mix(style->body.main_color, edgui_color_hsv_to_rgb(hue2, 90, 70), EDGUI_OPA_70);
    style->body.grad_color   = edgui_color_mix(style->body.grad_color, edgui_color_hsv_to_rgb(hue2, 90, 70), EDGUI_OPA_70);
    style->body.shadow.color = edgui_color_mix(style->body.shadow.color, edgui_color_hsv_to_rgb(hue2, 90, 70), EDGUI_OPA_60);

    style->text.color = edgui_color_mix(style->text.color, edgui_color_hsv_to_rgb(hue2, 90, 70), EDGUI_OPA_70);
#else
    style->body.border.opa   = EDGUI_OPA_COVER;
