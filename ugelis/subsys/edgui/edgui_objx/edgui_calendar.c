 ...
 *  STATIC PROTOTYPES
 **********************/
static bool edgui_calendar_design(edgui_obj_t * calendar, const edgui_area_t * mask, edgui_design_mode_t mode);
static edgui_res_t edgui_calendar_signal(edgui_obj_t * calendar, edgui_signal_t sign, void * param);
static bool calculate_touched_day(edgui_obj_t * calendar, const edgui_point_t * touched_point);
static edgui_coord_t get_header_height(edgui_obj_t * calendar);
static edgui_coord_t get_day_names_height(edgui_obj_t * calendar);
static void draw_header(edgui_obj_t * calendar, const edgui_area_t * mask);
static void draw_day_names(edgui_obj_t * calendar, const edgui_area_t * mask);
static void draw_days(edgui_obj_t * calendar, const edgui_area_t * mask);
static uint8_t get_day_of_week(uint32_t year, uint32_t month, uint32_t day);
static bool is_highlighted(edgui_obj_t * calendar, int32_t year, int32_t month, int32_t day);
static const char * get_day_name(edgui_obj_t * calendar, uint8_t day);
static const char * get_month_name(edgui_obj_t * calendar, int32_t month);
...
{
    EDGUI_LOG_TRACE("calendar create started");

    /*Create the ancestor of calendar*/
    edgui_obj_t * new_calendar = edgui_obj_create(par, copy);
    edgui_mem_assert(new_calendar);
    if(new_calendar == NULL) return NULL;

    /*Allocate the calendar type specific extended data*/
    edgui_calendar_ext_t * ext = edgui_obj_allocate_ext_attr(new_calendar, sizeof(edgui_calendar_ext_t));
    edgui_mem_assert(ext);
    if(ext == NULL) return NULL;
    if(ancestor_signal == NULL) ancestor_signal = edgui_obj_get_signal_cb(new_calendar);
    if(ancestor_design == NULL) ancestor_design = edgui_obj_get_design_cb(new_calendar);
    ...
    ext->style_header           = &edgui_style_plain_color;
    ext->style_header_pr        = &edgui_style_pretty_color;
    ext->style_highlighted_days = &edgui_style_plain_color;
    ext->style_inactive_days    = &edgui_style_btn_ina;
    ext->style_week_box         = &edgui_style_plain_color;
    ext->style_today_box        = &edgui_style_pretty_color;
    ext->style_day_names        = &edgui_style_pretty;

    /*The signal and design functions are not copied so set them here*/
    edgui_obj_set_signal_cb(new_calendar, edgui_calendar_signal);
    edgui_obj_set_design_cb(new_calendar, edgui_calendar_design);

    /*Init the new calendar calendar*/
    if(copy == NULL) {
        edgui_obj_set_size(new_calendar, EDGUI_DPI * 2, EDGUI_DPI * 2);
        edgui_obj_set_style(new_calendar, &edgui_style_pretty);

        edgui_theme_t * th = edgui_theme_get_current();
        if(th) {
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_BG, th->style.calendar.bg);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_HEADER, th->style.calendar.header);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_HEADER_PR, th->style.calendar.header_pr);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_DAY_NAMES, th->style.calendar.day_names);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_WEEK_BOX, th->style.calendar.week_box);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_TODAY_BOX, th->style.calendar.today_box);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_HIGHLIGHTED_DAYS,
                                  th->style.calendar.highlighted_days);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_INACTIVE_DAYS, th->style.calendar.inactive_days);
        } else {
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_BG, &edgui_style_pretty);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_HEADER, ext->style_header);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_HEADER_PR, ext->style_header_pr);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_DAY_NAMES, ext->style_day_names);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_WEEK_BOX, ext->style_week_box);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_TODAY_BOX, ext->style_today_box);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_HIGHLIGHTED_DAYS, ext->style_highlighted_days);
            edgui_calendar_set_style(new_calendar, EDGUI_CALENDAR_STYLE_INACTIVE_DAYS, ext->style_inactive_days);
        }
    }
    /*Copy an existing calendar*/
    else {
        edgui_calendar_ext_t * copy_ext = edgui_obj_get_ext_attr(copy);
        ...
        edgui_obj_refresh_style(new_calendar);
    }

    EDGUI_LOG_INFO("calendar created");
...
{
    edgui_calendar_ext_t * ext = edgui_obj_get_ext_attr(calendar);
    ext->today.year         = today->year;
    ext->today.month        = today->month;
    ext->today.day          = today->day;

    edgui_obj_invalidate(calendar);
}
...
void edgui_calendar_set_style(edgui_obj_t * calendar, edgui_calendar_style_t type, const edgui_style_t * style)
{
    ...
    switch(type) {
        case EDGUI_CALENDAR_STYLE_BG: edgui_obj_set_style(calendar, style); break;
        case EDGUI_CALENDAR_STYLE_DAY_NAMES: ext->style_day_names = style; break;
        case EDGUI_CALENDAR_STYLE_HEADER: ext->style_header = style; break;
        case EDGUI_CALENDAR_STYLE_HEADER_PR: ext->style_header_pr = style; break;
        case EDGUI_CALENDAR_STYLE_HIGHLIGHTED_DAYS: ext->style_highlighted_days = style; break;
        case EDGUI_CALENDAR_STYLE_INACTIVE_DAYS: ext->style_inactive_days = style; break;
        case EDGUI_CALENDAR_STYLE_TODAY_BOX: ext->style_today_box = style; break;
        case EDGUI_CALENDAR_STYLE_WEEK_BOX: ext->style_week_box = style; break;
    }
...
const edgui_style_t * edgui_calendar_get_style(const edgui_obj_t * calendar, edgui_calendar_style_t type)
{
    ...
    switch(type) {
        case EDGUI_CALENDAR_STYLE_BG: style = edgui_obj_get_style(calendar); break;
        case EDGUI_CALENDAR_STYLE_HEADER: style = ext->style_header; break;
        case EDGUI_CALENDAR_STYLE_HEADER_PR: style = ext->style_header_pr; break;
        case EDGUI_CALENDAR_STYLE_DAY_NAMES: style = ext->style_day_names; break;
        case EDGUI_CALENDAR_STYLE_HIGHLIGHTED_DAYS: style = ext->style_highlighted_days; break;
        case EDGUI_CALENDAR_STYLE_INACTIVE_DAYS: style = ext->style_inactive_days; break;
        case EDGUI_CALENDAR_STYLE_WEEK_BOX: style = ext->style_week_box; break;
        case EDGUI_CALENDAR_STYLE_TODAY_BOX: style = ext->style_today_box; break;
...
static bool edgui_calendar_design(edgui_obj_t * calendar, const edgui_area_t * mask, edgui_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        edgui_opa_t opa_scale = edgui_obj_get_opa_scale(calendar);
        edgui_draw_rect(&calendar->coords, mask, edgui_calendar_get_style(calendar, EDGUI_CALENDAR_STYLE_BG), opa_scale);
...
static edgui_res_t edgui_calendar_signal(edgui_obj_t * calendar, edgui_signal_t sign, void * param)
{
    ...
    if(sign == EDGUI_SIGNAL_CLEANUP) {
        ...
        edgui_calendar_ext_t * ext = edgui_obj_get_ext_attr(calendar);
        edgui_area_t header_area;
        edgui_area_copy(&header_area, &calendar->coords);
        ...
        if(edgui_area_is_point_on(&header_area, &p)) {
            if(p.x < header_area.x1 + edgui_area_get_width(&header_area) / 2) {
                if(ext->btn_pressing != -1) edgui_obj_invalidate(calendar);
                ext->btn_pressing = -1;
            } else {
                if(ext->btn_pressing != 1) edgui_obj_invalidate(calendar);
                ext->btn_pressing = 1;
        ...
        /*If a day is pressed save it*/
            ...
            if(ext->btn_pressing != 0) edgui_obj_invalidate(calendar);
            ext->btn_pressing = 0;
        }
        ...
        if(ext->btn_pressing < 0) {
            ...
            res = edgui_event_send(calendar, EDGUI_EVENT_VALUE_CHANGED, NULL);
            if(res != EDGUI_RES_OK) return res;
        }
        ...
        uint8_t i;
        for(i = 0; i < EDGUI_MAX_ANCESTOR_NUM - 1; i++) { /*Find the last set date*/
            if(buf->type[i] == NULL) break;
        }
        buf->type[i] = "edgui_calendar";
    }
...
static bool calculate_touched_day(edgui_obj_t * calendar, const edgui_point_t * touched_point)
{
    edgui_area_t days_area;
    edgui_area_copy(&days_area, &calendar->coords);
    const edgui_style_t * style_bg = edgui_calendar_get_style(calendar, EDGUI_CALENDAR_STYLE_BG);
    ...
    if(edgui_area_is_point_on(&days_area, touched_point)) {
        edgui_coord_t w  = (days_area.x2 - days_area.x1 + 1) / 7;
        edgui_coord_t h  = (days_area.y2 - days_area.y1 + 1) / 6;
        uint8_t x_pos = 0;
...
static edgui_coord_t get_header_height(edgui_obj_t * calendar)
{
    ...
    return edgui_font_get_line_height(ext->style_header->text.font) + ext->style_header->body.padding.top +
...
static edgui_coord_t get_day_names_height(edgui_obj_t * calendar)
{
    ...
    return edgui_font_get_line_height(ext->style_day_names->text.font) + ext->style_day_names->body.padding.top +
...
static void draw_header(edgui_obj_t * calendar, const edgui_area_t * mask)
{
    ...
    edgui_draw_rect(&header_area, mask, ext->style_header, opa_scale);

    /*Add the year + month name*/
    char txt_buf[64];
    edgui_utils_num_to_str(ext->showed_date.year, txt_buf);
    txt_buf[4] = ' ';
    txt_buf[5] = '\0';
    strcpy(&txt_buf[5], get_month_name(calendar, ext->showed_date.month));
    header_area.y1 += ext->style_header->body.padding.top;
    edgui_draw_label(&header_area, mask, ext->style_header, opa_scale, txt_buf, EDGUI_TXT_FLAG_CENTER, NULL, -1, -1, NULL);

    /*Add the left arrow*/
    const edgui_style_t * arrow_style = ext->btn_pressing < 0 ? ext->style_header_pr : ext->style_header;
    header_area.x1 += ext->style_header->body.padding.left;
    edgui_draw_label(&header_area, mask, arrow_style, opa_scale, EDGUI_SYMBOL_LEFT, EDGUI_TXT_FLAG_NONE, NULL, -1, -1, NULL);
    ...
    header_area.x1 = header_area.x2 - ext->style_header->body.padding.right -
                     edgui_txt_get_width(EDGUI_SYMBOL_RIGHT, strlen(EDGUI_SYMBOL_RIGHT), arrow_style->text.font,
                                      arrow_style->text.line_space, EDGUI_TXT_FLAG_NONE);
    edgui_draw_label(&header_area, mask, arrow_style, opa_scale, EDGUI_SYMBOL_RIGHT, EDGUI_TXT_FLAG_NONE, NULL, -1, -1, NULL);
}
...
static void draw_day_names(edgui_obj_t * calendar, const edgui_area_t * mask)
{
    ...
    edgui_coord_t l_pad = ext->style_day_names->body.padding.left;
    edgui_coord_t w =
        edgui_obj_get_width(calendar) - ext->style_day_names->body.padding.left - ext->style_day_names->body.padding.right;
    edgui_coord_t box_w = w / 7;
    edgui_area_t label_area;
    label_area.y1 = calendar->coords.y1 + get_header_height(calendar) + ext->style_day_names->body.padding.top;
    label_area.y2 = label_area.y1 + edgui_font_get_line_height(ext->style_day_names->text.font);
    uint32_t i;
    for(i = 0; i < 7; i++) {
        label_area.x1 = calendar->coords.x1 + (w * i) / 7 + l_pad;
        label_area.x2 = label_area.x1 + box_w - 1;
        edgui_draw_label(&label_area, mask, ext->style_day_names, opa_scale, get_day_name(calendar, i), EDGUI_TXT_FLAG_CENTER,
                      NULL, -1, -1, NULL);
    }
}
...
static void draw_days(edgui_obj_t * calendar, const edgui_area_t * mask)
{
    ...
    edgui_opa_t opa_scale = edgui_obj_get_opa_scale(calendar);
    label_area.y1      = calendar->coords.y1 + get_header_height(calendar) + ext->style_day_names->body.padding.top +
                    edgui_font_get_line_height(ext->style_day_names->text.font) +
                    ext->style_day_names->body.padding.bottom;
    label_area.y2 = label_area.y1 + edgui_font_get_line_height(style_bg->text.font);

    edgui_coord_t w          = edgui_obj_get_width(calendar) - style_bg->body.padding.left - style_bg->body.padding.right;
    edgui_coord_t h          = calendar->coords.y2 - label_area.y1 - style_bg->body.padding.bottom;
    edgui_coord_t box_w      = w / 7;
    edgui_coord_t vert_space = (h - (6 * edgui_font_get_line_height(style_bg->text.font))) / 5;
    ...
    for(week = 0; week < 6; week++) {
        ...
        if(month_of_today_shown &&
           ((draw_state == DAY_DRAW_ACT_MONTH && ext->today.day >= day_cnt && ext->today.day < day_cnt + 7) ||
            ...
            edgui_area_t week_box_area;
            edgui_area_copy(&week_box_area, &label_area); /*'label_area' is already set for this row*/
            week_box_area.x1 =
            ...
            edgui_draw_rect(&week_box_area, mask, ext->style_week_box, opa_scale);
        ...
        for(day = 0; day < 7; day++) {
            ...
            if(draw_state == DAY_DRAW_ACT_MONTH && month_of_today_shown && ext->today.day == day_cnt) {
                edgui_area_t today_box_area;
                edgui_area_copy(&today_box_area, &label_area);
                ...
                edgui_draw_rect(&today_box_area, mask, ext->style_today_box, opa_scale);
            }

            /*Get the final style : highlighted/week box/today box/normal*/
            const edgui_style_t * final_style;
            ...
            edgui_utils_num_to_str(day_cnt, buf);
            edgui_draw_label(&label_area, mask, final_style, opa_scale, buf, EDGUI_TXT_FLAG_CENTER, NULL, -1, -1, NULL);
        ...
        label_area.y1 += vert_space + edgui_font_get_line_height(style_bg->text.font);
        label_area.y2 += vert_space + edgui_font_get_line_height(style_bg->text.font);
    }
}
