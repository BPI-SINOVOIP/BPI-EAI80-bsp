...
{
	SEGGER_SYSVIEW_SendSysDesc("N=uGelisSysViewSample");
	SEGGER_SYSVIEW_SendSysDesc("D=" CONFIG_BOARD " "
				   CONFIG_SOC " " CONFIG_ARCH);
	SEGGER_SYSVIEW_SendSysDesc("O=uGelis");
}
...
static void sysview_api_send_task(const struct k_thread *thr, void *udata)
{
	...
	/* NOTE: struct k_thread is inside the stack on uGelis 1.7.
...
static u32_t ugelis_to_sysview(int event_type)
{
...
#define ENABLE_SYSVIEW_EVENT(event) \
	(MUST_LOG(event) ? ugelis_to_sysview(event ## _EVENT_ID) : 0)
