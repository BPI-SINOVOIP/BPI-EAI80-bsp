...
/**
 * cipher key select structure definition
 */
enum cipher_keyfrom {
	CRYPTO_CIPHER_KEYFROM_REG      = 0,   /**< input key is from register */
	CRYPTO_CIPHER_KEYFROM_OTP      = 1,   /**< input key is from OTP */
	CRYPTO_CIPHER_KEYFROM_KEYBUF   = 2,   /**< input key is from key buffer */
} ;

/**
 * CRYP data type structure definition
 */
enum cipher_datatype
{
	CRYP_CIPHER_DATATYPE_32B      = 0,          /**< 32bit data type */
	CRYP_CIPHER_DATATYPE_16B      = 1,          /**< 16bit data type */
	CRYP_CIPHER_DATATYPE_8B       = 2,          /**< 8bit data type */
	CRYP_CIPHER_DATATYPE_1B       = 3           /**< 1bit data type */
};
...
struct cipher_ctx {
	...
	union {
		/* Cryptographic key to be used in this session when key from regester */
		u8_t *bit_stream;
		/* For cases where  key is protected and is not
		 * available to caller, used when key from OTP or keybuf
		 */
		void *handle;
	} key;
	...
	u16_t  keylen;
	/*
	 * Specify where the keys stored,this parameter can be a value of @ref cipher_keyfrom
	 */
	u8_t keyfrom; 
	/*
	 * Specify the data type, this parameter can be a value of @ref cipher_datatype
	 */
	u8_t data_type;
	/*
	 * enable DMA to transfer data or not
	 */
	u8_t enable_dma;
