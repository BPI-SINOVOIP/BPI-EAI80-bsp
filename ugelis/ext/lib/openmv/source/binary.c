...
#if 1//def IMLIB_ENABLE_BINARY_OPS
void imlib_binary(image_t *out, image_t *img, list_t *thresholds, bool invert, bool zero, image_t *mask)
{
    ...
    bmp.data = (uint8_t *)malloc(image_size(&bmp));
    memset(bmp.data, 0, image_size(&bmp));
...
void imlib_top_hat(image_t *img, int ksize, int threshold, image_t *mask)
{
    ...
    temp.data = (uint8_t *)malloc(image_size(img));
    memset(temp.data, 0, (image_size(img)));
