...
{
    zbar_video_t *vdo = zbar_calloc(1, sizeof(zbar_video_t));
    if(!vdo)
        return(NULL);
    ...
    vdo->images = zbar_calloc(ZBAR_VIDEO_IMAGES_MAX, sizeof(zbar_image_t*));
    if(!vdo->images) {
...
void zbar_video_destroy (zbar_video_t *vdo)
{
    ...
    if(vdo->images) {
        ...
        for(i = 0; i < ZBAR_VIDEO_IMAGES_MAX; i++)
            if(vdo->images[i])
                zbar_free(vdo->images[i]);
        zbar_free(vdo->images);
    }
...
static inline int video_init_images (zbar_video_t *vdo)
{
    ...
    if(vdo->iomode != VIDEO_MMAP) {
        ...
        vdo->buf = zbar_malloc(vdo->buflen);
        if(!vdo->buf)
    ...
    if(img) {
        ...
        if(vdo->num_images < 2) {
            ...
            if(!img) {
                ...
                img->height = vdo->height;
                img->datalen = vdo->datalen;
                img->data = zbar_malloc(vdo->datalen);
            }
