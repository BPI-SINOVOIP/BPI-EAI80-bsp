...
static void ugelis_os_mgmt_reset_cb(struct k_timer *timer);
static void ugelis_os_mgmt_reset_work_handler(struct k_work *work);

static K_TIMER_DEFINE(ugelis_os_mgmt_reset_timer,
                      ugelis_os_mgmt_reset_cb, NULL);

K_WORK_DEFINE(ugelis_os_mgmt_reset_work, ugelis_os_mgmt_reset_work_handler);

#ifdef CONFIG_THREAD_MONITOR
static const struct k_thread *
ugelis_os_mgmt_task_at(int idx)
{
...
os_mgmt_impl_task_info(int idx, struct os_mgmt_task_info *out_info)
{
    ...
    thread = ugelis_os_mgmt_task_at(idx);
    if (thread == NULL) {
...
static void
ugelis_os_mgmt_reset_work_handler(struct k_work *work)
{
    sys_reboot(SYS_REBOOT_WARM);
}

static void
ugelis_os_mgmt_reset_cb(struct k_timer *timer)
{
    /* Reboot the system from the system workqueue thread. */
    k_work_submit(&ugelis_os_mgmt_reset_work);
}

int
os_mgmt_impl_reset(unsigned int delay_ms)
{
    k_timer_start(&ugelis_os_mgmt_reset_timer, K_MSEC(delay_ms), 0);
    return 0;
}
