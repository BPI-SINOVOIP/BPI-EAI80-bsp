...
def generate_node_definitions(yaml_list):
    ...
    node_address = chosen.get('ugelis,flash', 'dummy-flash')
    flash.extract(node_address, yaml_list, 'ugelis,flash', None, defs, 'FLASH')
    node_address = chosen.get('ugelis,code-partition', node_address)
    flash.extract(node_address, yaml_list, 'ugelis,code-partition', None,
                  defs, 'FLASH')
