操作步骤：
	1.进入ugelis 目录下:source ugelis-env.sh
	2.进入到相应工程目录下，cmake一下：	mkdir build
										cd build/
										cmake ../
	3.进入生成的build目录下，make一下:	cd build/
										make
	4.build目录下：mkfsimg 会在ugelis/data目录下生成fat16.img文件 (如遇到解释器错误，运行sed -i 's/\r$//' mkfsimg.sh)
	5.build目录下：mkimg，最终会在当前目录生成mkimg.img文件 (如遇到解释器错误，运行sed -i 's/\r$//' mkimg.sh)
