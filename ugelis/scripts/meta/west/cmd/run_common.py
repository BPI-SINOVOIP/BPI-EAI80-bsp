...
from .. import log
from .. import util
from ..runner import get_runner_cls, uGelisBinaryRunner
...
def add_parser_common(parser_adder, command):
    ...
    group.add_argument('-c', '--cmake-cache', default=cmake.DEFAULT_CACHE,
                       ...
                       configuration (this is generated by the uGelis
    ...
    group = parser.add_argument_group(
        ...
        These values usually come from the uGelis build system itself
...
def cached_runner_config(build_dir, cache):
    ...
    board_dir = cache['UGELIS_RUNNER_CONFIG_BOARD_DIR']
    kernel_elf = cache['UGELIS_RUNNER_CONFIG_KERNEL_ELF']
    kernel_hex = cache['UGELIS_RUNNER_CONFIG_KERNEL_HEX']
    kernel_bin = cache['UGELIS_RUNNER_CONFIG_KERNEL_BIN']
    gdb = cache.get('UGELIS_RUNNER_CONFIG_GDB')
    openocd = cache.get('UGELIS_RUNNER_CONFIG_OPENOCD')
    openocd_search = cache.get('UGELIS_RUNNER_CONFIG_OPENOCD_SEARCH')
...
def do_run_common(command, args, runner_args, cached_runner_var):
    ...
    cached_runner_args = cache.get_list(
        'UGELIS_RUNNER_ARGS_{}'.format(cmake.make_c_identifier(runner)))
...
def _dump_context(command, args, runner_args, cached_runner_var):
    ...
    all_cls = {cls.name(): cls for cls in uGelisBinaryRunner.get_runners() if
               command.name in cls.capabilities().commands}
    available = [r for r in cache.get_list('UGELIS_RUNNERS') if r in all_cls]
    available_cls = {r: all_cls[r] for r in available if r in all_cls}

    default_runner = cache.get(cached_runner_var)
    cfg = cached_runner_config(build_dir, cache)

    log.inf('All uGelis runners which support {}:'.format(command.name))
...
def _dump_one_runner_info(cache, args, build_dir, indent):
    ...
    available = runner in cache.get_list('UGELIS_RUNNERS')
    cfg = cached_runner_config(build_dir, cache)
...
def _get_runner_args(cache, runner):
    ...
    args_var = 'UGELIS_RUNNER_ARGS_{}'.format(runner_ident)
