...
# uGelis applications, e.g. ugelis/samples/hello_world/CMakeLists.txt
# must start with the line:
#
# include($ENV{UGELIS_BASE}/cmake/app/boilerplate.cmake NO_POLICY_SCOPE)
#
# It exists to reduce boilerplate code that uGelis expects to be in
...
define_property(GLOBAL PROPERTY UGELIS_LIBS
    BRIEF_DOCS "Global list of all uGelis CMake libs that should be linked in"
    FULL_DOCS  "Global list of all uGelis CMake libs that should be linked in.
ugelis_library() appends libs to this list.")
set_property(GLOBAL PROPERTY UGELIS_LIBS "")

define_property(GLOBAL PROPERTY UGELIS_INTERFACE_LIBS
    BRIEF_DOCS "Global list of all uGelis interface libs that should be linked in."
    FULL_DOCS  "Global list of all uGelis interface libs that should be linked in.
ugelis_interface_library_named() appends libs to this list.")
set_property(GLOBAL PROPERTY UGELIS_INTERFACE_LIBS "")

define_property(GLOBAL PROPERTY GENERATED_KERNEL_OBJECT_FILES
  BRIEF_DOCS "Object files that are generated after uGelis has been linked once."
  FULL_DOCS "\
Object files that are generated after uGelis has been linked once.\
...
define_property(GLOBAL PROPERTY GENERATED_KERNEL_SOURCE_FILES
  BRIEF_DOCS "Source files that are generated after uGelis has been linked once."
  FULL_DOCS "\
Object files that are generated after uGelis has been linked once.\
...
set(__build_dir ${CMAKE_CURRENT_BINARY_DIR}/ugelis)

set(PROJECT_BINARY_DIR ${__build_dir})

# CMake's 'project' concept has proven to not be very useful for uGelis
# due in part to how uGelis is organized and in part to it not fitting well
# with cross compilation.
# CMake therefore tries to rely as little as possible on project()
# and its associated variables, e.g. PROJECT_SOURCE_DIR.
# It is recommended to always use UGELIS_BASE instead of PROJECT_SOURCE_DIR
# when trying to reference ENV${UGELIS_BASE}.
set(PROJECT_SOURCE_DIR $ENV{UGELIS_BASE})

# Convert path to use the '/' separator
string(REPLACE "\\" "/" PROJECT_SOURCE_DIR ${PROJECT_SOURCE_DIR})

set(UGELIS_BINARY_DIR ${PROJECT_BINARY_DIR})
set(UGELIS_BASE ${PROJECT_SOURCE_DIR})

set(AUTOCONF_H ${__build_dir}/include/generated/autoconf.h)
# Re-configure (Re-execute all CMakeLists.txt code) when autoconf.h changes
set_property(DIRECTORY APPEND PROPERTY CMAKE_CONFIGURE_DEPENDS ${AUTOCONF_H})

include(${UGELIS_BASE}/cmake/extensions.cmake)

find_package(PythonInterp 3.4)

include(${UGELIS_BASE}/cmake/ccache.cmake)
...
add_custom_target(
  pristine
  COMMAND ${CMAKE_COMMAND} -P ${UGELIS_BASE}/cmake/pristine.cmake
...
if(MSYS)
  execute_process(
    COMMAND
    ${PYTHON_EXECUTABLE} ${UGELIS_BASE}/scripts/check_host_is_ok.py
    RESULT_VARIABLE ret
    )
...
# Use BOARD to search ugelis/boards/** for a _defconfig file,
# e.g. ugelis/boards/arm/96b_carbon_nrf51/96b_carbon_nrf51_defconfig. When
# found, use that path to infer the ARCH we are building for.
if(NOT BOARD_ROOT)
  set(BOARD_ROOT ${UGELIS_BASE})
endif()
...
include(${UGELIS_BASE}/cmake/version.cmake)
include(${UGELIS_BASE}/cmake/host-tools.cmake)
include(${UGELIS_BASE}/cmake/kconfig.cmake)
include(${UGELIS_BASE}/cmake/toolchain.cmake)

find_package(Git QUIET)
if(GIT_FOUND)
  execute_process(COMMAND ${GIT_EXECUTABLE} describe
    WORKING_DIRECTORY ${UGELIS_BASE}
...
include(${UGELIS_BASE}/cmake/dts.cmake)
...
ugelis_library_named(app)

add_subdirectory(${UGELIS_BASE} ${__build_dir})

# Link 'app' with the uGelis interface libraries.
#
...
# done after 'add_subdirectory(${UGELIS_BASE} ${__build_dir})'
# because interface libraries are defined while processing that
# subdirectory.
get_property(UGELIS_INTERFACE_LIBS_PROPERTY GLOBAL PROPERTY UGELIS_INTERFACE_LIBS)
foreach(boilerplate_lib ${UGELIS_INTERFACE_LIBS_PROPERTY})
...
set(MKIMG_MAKEFILE ${UGELIS_BASE}/scripts/mkimg/mkimgfile)
set(MKIMG_SHELL ${UGELIS_BASE}/scripts/mkimg/mkimg.sh)
set(MKFSIMG_SHELL ${UGELIS_BASE}/scripts/mkimg/mkfsimg.sh)

execute_process(
    COMMAND ${CMAKE_COMMAND} -E copy ${MKIMG_MAKEFILE} ${CMAKE_CURRENT_BINARY_DIR}
    COMMAND ${CMAKE_COMMAND} -E copy ${MKIMG_SHELL} ${CMAKE_CURRENT_BINARY_DIR}
    COMMAND ${CMAKE_COMMAND} -E copy ${MKFSIMG_SHELL} ${CMAKE_CURRENT_BINARY_DIR}
)
