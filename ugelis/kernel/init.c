...
void _bss_zero(void)
{
	...
	__bss_start = (char *)&Image$$BSS_REGION$$Base;
	__bss_end	= (char *)&Image$$NOINIT_REGION$$Base;
...
{
#ifdef __USE_KEIL_
	__data_rom_start = (char *)&Load$$DATA_START_REGION$$Base;
	__data_ram_start = (char *)&Image$$DATA_START_REGION$$Base;
	__data_ram_end = (char *)(((unsigned int)&Image$$DATA_END_REGION$$Base) + ((unsigned int)&Load$$DATA_END_REGION$$Length));

	memcpy(__data_ram_start, __data_rom_start, (u32_t) __data_ram_end - (u32_t) __data_ram_start);
#else
...
static void prepare_multithreading(struct k_thread *dummy_thread)
{
	...
	g_kernel.cpus[0].idle_thread = _idle_thread;
#endif

#if defined(CONFIG_SMP) && CONFIG_MP_NUM_CPUS > 1
	init_idle_thread(_idle_thread1, _idle_stack1);
	g_kernel.cpus[1].idle_thread = _idle_thread1;
	g_kernel.cpus[1].id = 1;
	g_kernel.cpus[1].irq_stack = K_THREAD_STACK_BUFFER(_interrupt_stack1)
		+ CONFIG_ISR_STACK_SIZE;
#endif

#if defined(CONFIG_SMP) && CONFIG_MP_NUM_CPUS > 2
	init_idle_thread(_idle_thread2, _idle_stack2);
	g_kernel.cpus[2].idle_thread = _idle_thread2;
	g_kernel.cpus[2].id = 2;
	g_kernel.cpus[2].irq_stack = K_THREAD_STACK_BUFFER(_interrupt_stack2)
		+ CONFIG_ISR_STACK_SIZE;
#endif

#if defined(CONFIG_SMP) && CONFIG_MP_NUM_CPUS > 3
	init_idle_thread(_idle_thread3, _idle_stack3);
	g_kernel.cpus[3].idle_thread = _idle_thread3;
	g_kernel.cpus[3].id = 3;
	g_kernel.cpus[3].irq_stack = K_THREAD_STACK_BUFFER(_interrupt_stack3)
		+ CONFIG_ISR_STACK_SIZE;
#endif
