...
static struct k_thread *next_up(void)
{
	 ...
	 */
	struct k_thread *th = _priq_run_best(&g_kernel.ready_q.runq);
...
static void update_cache(int preempt_ok)
{
	...
	if (should_preempt(th, preempt_ok)) {
		g_kernel.ready_q.cache = th;
	} else {
		g_kernel.ready_q.cache = _current;
	}
...
{
	k_spinlock_key_t __i,__key; 
	__i.key = 0;
	for (__key = k_spin_lock(&sched_lock);	!__i.key;k_spin_unlock(&sched_lock, __key), __i.key = 1)
	{
		_priq_run_add(&g_kernel.ready_q.runq, thread);
...
void _move_thread_to_end_of_prio_q(struct k_thread *thread)
{
	...
	for (__key = k_spin_lock(&sched_lock);	!__i.key;k_spin_unlock(&sched_lock, __key), __i.key = 1)
	{
		_priq_run_remove(&g_kernel.ready_q.runq, thread);
		_priq_run_add(&g_kernel.ready_q.runq, thread);
		_mark_thread_as_queued(thread);
		update_cache(0);
	}
...
struct k_thread *_get_next_ready_thread(void)
{
	...
	k_spinlock_key_t __i,__key; 
	__i.key = 0;
	for (__i = {},__key = k_spin_lock(&sched_lock);	!__i.key;k_spin_unlock(&sched_lock, __key), __i.key = 1)
	{
		ret = next_up();
	}
...
void _priq_rb_add(struct _priq_rb *pq, struct k_thread *thread)
{
	...
	if (!pq->next_order_key) {
		struct _rb_foreach __f; 
		do
		{
			__f.stack   = alloca((&pq->tree)->max_depth * sizeof(struct rbnode *));
			__f.is_left = alloca((&pq->tree)->max_depth * sizeof(char));
			__f.top     = -1;				
			t 	= CONTAINER_OF(_rb_foreach_next(&pq->tree, &__f),struct k_thread, base.qnode_rb);
			if(t == NULL)
			{
				break;
			}
...
int _is_thread_time_slicing(struct k_thread *thread)
{
	...
	for (__key = k_spin_lock(&sched_lock);	!__i.key;k_spin_unlock(&sched_lock, __key), __i.key = 1)
	{
		struct k_thread *next = _priq_run_best(&g_kernel.ready_q.runq);
...
void _sched_init(void)
{
	...
	sys_dlist_init(&g_kernel.ready_q.runq);
#endif

#ifdef CONFIG_SCHED_SCALABLE
	g_kernel.ready_q.runq = (struct _priq_rb) {
		.tree = {
...
#ifdef CONFIG_SCHED_MULTIQ
	int i;
	for (i = 0; i < ARRAY_SIZE(g_kernel.ready_q.runq.queues); i++) {
		sys_dlist_init(&g_kernel.ready_q.runq.queues[i]);
	}
#endif
}
...
void _impl_k_thread_deadline_set(k_tid_t tid, int deadline)
{
	...
	for (__key = k_spin_lock(&sched_lock);	!__i.key;k_spin_unlock(&sched_lock, __key), __i.key = 1)
	{
		...
		if (_is_thread_queued(th)) {
			_priq_run_remove(&g_kernel.ready_q.runq, th);
			_priq_run_add(&g_kernel.ready_q.runq, th);
		}
	}
}
...
void _impl_k_yield(void)
{
	...
	if (!_is_idle(_current)) {
		...
		for (__key = k_spin_lock(&sched_lock);	!__i.key;k_spin_unlock(&sched_lock, __key), __i.key = 1)
		{
			_priq_run_remove(&g_kernel.ready_q.runq, _current);
			_priq_run_add(&g_kernel.ready_q.runq, _current);
			update_cache(1);
		}
	}
