...
static u32_t ugelis_to_posix_priority(s32_t z_prio, int *policy)
{
	u32_t prio;
...
static s32_t posix_to_ugelis_priority(u32_t priority, int policy)
{
	s32_t prio;

	if (policy == SCHED_FIFO) {
		/* uGelis COOP priority starts from -1 */
		prio =  -1 * (priority + 1);
	} else {
...
static void ugelis_thread_wrapper(void *arg1, void *arg2, void *arg3)
{
		   ...
		   void *(*threadroutine)(void *), void *arg)
{
	 ...
	 * attrib as NULL but ugelis needs it initialized with valid stack.
	 */
	if (!attr || !attr->initialized || !attr->stack || !attr->stacksize) {
		return EINVAL;
	}

	prio = posix_to_ugelis_priority(attr->priority, attr->schedpolicy);
			  ...
			  const struct sched_param *param)
{
	...
	new_prio = posix_to_ugelis_priority(param->priority, policy);
			  ...
			  struct sched_param *param)
{
	...
	param->priority = ugelis_to_posix_priority(priority, policy);
	return 0;
}
