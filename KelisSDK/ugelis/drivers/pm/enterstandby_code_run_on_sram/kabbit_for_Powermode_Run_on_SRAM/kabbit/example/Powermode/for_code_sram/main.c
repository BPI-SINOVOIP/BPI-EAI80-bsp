...
void pm_shutdown_wkpin_xpb12_config(struct pm_frame_wkup_pin_cfg *wkup_pin_cfg)
{
    ...
    while (!(IO_BIT_GET(HW32_REG(RTC_BASE + 0x08), 0x1 << 22)));
    IO_BITMASK_SET(*(volatile unsigned long *)(RTC_BASE + 0x58), 7 << 10, (3 << 10) | (PM_CFG->pm_shutdown_status_pin_inv) << 12);
    while (!(IO_BIT_GET(HW32_REG(RTC_BASE + 0x08), 0x1 << 22)));
}
...
void pm_shutdown_wkpin_xpb13_config(struct pm_frame_wkup_pin_cfg *wkup_pin_cfg)
{
    ...
    while (!(IO_BIT_GET(HW32_REG(RTC_BASE + 0x08), 0x1 << 22)));
    IO_BITMASK_SET(*(volatile unsigned long *)(RTC_BASE + 0x58), 7 << 10, (1 << 10) | (PM_CFG->pm_shutdown_status_pin_inv) << 12);
    while (!(IO_BIT_GET(HW32_REG(RTC_BASE + 0x08), 0x1 << 22)));
}

void sdram_pin_output_pd(void)
{
    GPIO_PinConfig_T GPIO_InitStruct;
    ...
    //    memset(&GPIO_InitStruct, 0, sizeof(GPIO_PinConfig_T));
    GPIO_InitStruct.pin =  0x007F;
    GPIO_InitStruct.mode = GPIO_MODE_OUTPUT;
    GPIO_InitStruct.pull = GPIO_PULLDOWN;
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

    //    memset(&GPIO_InitStruct, 0, sizeof(GPIO_PinConfig_T));
    GPIO_InitStruct.pin =  0xFFF0;
    GPIO_InitStruct.mode = GPIO_MODE_OUTPUT;
    GPIO_InitStruct.pull = GPIO_PULLDOWN;
    ...
    GPIO_InitStruct.pin =  0x01FF;
    GPIO_InitStruct.mode = GPIO_MODE_OUTPUT;
    GPIO_InitStruct.pull = GPIO_PULLDOWN;
    HAL_GPIO_Init(GPIOK, &GPIO_InitStruct);

    HW32_REG(0x40010004) &= 0x0000;
    HW32_REG(0x40011004) &= 0xFF80;
    HW32_REG(0x40018004) &= 0x000F;
    HW32_REG(0x40019004) &= 0x0000;
    HW32_REG(0x4001A004) &= 0xF700;
