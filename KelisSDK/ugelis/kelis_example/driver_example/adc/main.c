...
int adc_channel_sync_demo(int demo_sel)
{
    ...
    if (adc_dev == NULL)
    {
        printf("Failed to get ADC device.");
    ...
    while (cnt++ < CNT)
    {
        ...
        printf("Channel:%d Convert Success Value:%d\n", sample.channel_cfg[0].rchn_sel, convert_data);

        k_sleep(100);
    }
    printf("================Exit func[%s] Test=================\n\n\n", __FUNCTION__);

    return TC_PASS;
}

int adc_sequence_sync_demo(int demo_sel)
{
    unsigned int convert_data = 0;
    struct device *adc_dev;
...
{
    float  test = 123.555;
    printf("================Exit func[%s] Test=======%f==========\n\n\n", __FUNCTION__, test);
    adc_channel_sync_demo(0);
    adc_sequence_sync_demo(0);
    printf("================Exit func[%s] Test=================\n\n\n", __FUNCTION__);
}
