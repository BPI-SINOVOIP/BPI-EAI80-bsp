...
#define OUTPUT_MAXLEN       (1*1024*1024)
...
static int jpeg_processDU(jpeg_buf_t *jpeg_buf, signed char *CDU, float *fdtbl, int DC, const uint16_t (*HTDC)[2], const uint16_t (*HTAC)[2])
{
    ...
    for (int i = 0; i < 64; ++i)
    {
        DUQ[s_jpeg_ZigZag[i]] = (int)(DU[i] * fdtbl[i]);  //gm_roundf_c(DU[i]*fdtbl[i]);  // richard debug
...
void jpeg_write(image_t *img, const char *path, int quality)
{
    ...
    uint8_t *buffer = (unsigned char *)malloc_ext(0, 1024 * 100); // (uint8_t*)calloc(OUTPUT_MAXLEN,sizeof(uint8_t));
    image_t out;
    out.w = img->w;
    out.h = img->h;
    out.bpp = size;
...
int demo(void)
{
    ...
    img.data = 0;//&buf_rgb888[0]; // 320*240
