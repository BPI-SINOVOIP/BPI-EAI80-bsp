...
static void lcd_showchar(uint16_t x, uint16_t y, uint8_t num, uint8_t size, uint8_t mode, uint32_t color)
{
    ...
    for (t = 0; t < csize; t++)
    {
        ...
        for (t1 = 0; t1 < 8; t1++)
        {
            ...
            {
                lcd_draw_point(x, y, TEXT_BACK_COLOR);
            }
            temp <<= 1;
            y++;
            if (y >= PIC_HEIGHT)
...
static void LCD_Clear()
{
    ...
    for (j = 0; j < 272; j++)
    {
        start_addr = ((unsigned char *)ui_buf_addr) + (480 * j) * 4;
        memset(start_addr, 0xff, 480 * 4);
    }
}

const uint32_t POINT_COLOR_TBL[5] = {RED, GREEN, BLUE, BROWN, GRED};

void Load_Drow_Dialog(void)
{
    LCD_Clear(WHITE);
    POINT_COLOR = BLUE;
    lcd_draw_text(480 - 24, 0, 200, 16, 16, "RST", 0xFFFF0000);
    POINT_COLOR = RED;
}

static void gui_draw_hline(uint16_t x0, uint16_t y0, uint16_t len, uint32_t color)
{
    uint16_t i;
    if (len == 0)
    {
    ...
    for (i = x0; i < x0 + 2 * len; i++)
    {
        *(uint32_t *)((uint32_t)ui_buf_addr + ((480 * y0 + i) << 2)) = color;
    }
}

static void gui_fill_circle(uint16_t x0, uint16_t y0, uint16_t r, uint32_t color)
{
    uint32_t i;
