...
int coremark()
{
    ...
    printf("CoreMark Start Test!!!\n");



    /* first call any initializations needed */
    results[0].port.portable_id = 1;
...
#error "Please define a way to initialize a memory block."
    ...
    seedcrc = crc16_cormark(results[0].seed1, seedcrc);
    seedcrc = crc16_cormark(results[0].seed2, seedcrc);
    seedcrc = crc16_cormark(results[0].seed3, seedcrc);
    seedcrc = crc16_cormark(results[0].size, seedcrc);
    ...
    if (total_errors == 0)
    {
        ...
        if (known_id == 3)
        {
            ee_printf("RUN MHZ:%d  CoreMark 1.0 : %f / %s %s", (CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC / 1000000),
                      default_num_contexts * results[0].iterations / time_in_secs(total_time), COMPILER_VERSION, COMPILER_FLAGS);
