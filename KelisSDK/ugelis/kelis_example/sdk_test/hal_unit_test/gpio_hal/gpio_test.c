...
int gpio_input_output_demo(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        HAL_GPIO_PinWrite(GPIO_BIT_ADDR((i + 16)), GPIO_PIN_SET); /*GPIOB0,  B1*/
        ...
        HAL_GPIO_PinWrite(GPIO_BIT_ADDR((i + 16)), GPIO_PIN_RESET); /*GPIOB0,  B1*/
...
int gpio_filter_demo(void)
{
    ...
    HAL_GPIO_SetPinFilter(GPIOC10, &GPIO_InputGlitchFilter); /*GPIOC*/
    HAL_GPIO_PinWrite(GPIOA0, GPIO_PIN_SET); /*GPIOA*/

    while (i++ < 2)
    {
        if (HAL_GPIO_PinRead(GPIOC10)) /*GPIOC*/
        {
...
int gpio_interrupt_demo(void)
{
    ...
    while (cnt++ < CNT)
    {
        ...
        k_sleep(1000);
        HAL_GPIO_PinWrite(GPIOA0, GPIO_PIN_RESET);
        HAL_GPIO_PinWrite(GPIOA1, GPIO_PIN_RESET);
        k_sleep(1000);
    }
