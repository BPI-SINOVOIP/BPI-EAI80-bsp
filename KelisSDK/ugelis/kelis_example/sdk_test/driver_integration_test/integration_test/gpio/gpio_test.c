...
#define OUTPUT0_GPIO_PIN   4
#define OUTPUT1_GPIO_PIN   5
#define OUTPUT2_GPIO_PIN   2

#define SW0_INPUT_GPIO_PIN   4
#define SW1_INPUT_GPIO_PIN   5
...
int gpio_input_output_demo(void)
{
    ...
    gpio_pin_write(dev, OUTPUT0_GPIO_PIN, GPIO_PIN_RESET);
    gpio_pin_read(dev, SW1_INPUT_GPIO_PIN, &val0);
    if (val0 != GPIO_PIN_RESET)
    {
        err_flag = 1;
...
int gpio_interrupt_demo(void)
{
    ...
    gpio_pin_configure(dev, SW0_INPUT_GPIO_PIN, GPIO_PUD_PULL_DOWN | GPIO_MODE_IT_RISING);
    ...
    gpio_pin_disable_callback(dev, SW0_INPUT_GPIO_PIN);
    return TC_PASS;
}
