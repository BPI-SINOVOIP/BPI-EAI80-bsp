...
void uart_recv_data_callback(struct device *unused)
{
    uint32_t    index;
    u8_t        byte = 0;
    int         rx = 0;
    ...
    {
        rx = uart_fifo_read(uart_console_dev, &byte, 1);
        if (rx < 0)
        {
            return;
        }
...
static int msendstart = 1;
static int execute = 0;
static int capstart = 0;
        ...
        if (1 == g_frame_recv_finished)//g_frame_be_read is seted means g_frame_recv_finished is also seted
        {
            ...
            if (TRUE == b_check_rxdata_valid(g_rxData, MAX_RX_BUF))
            {
                ...
                switch (g_rxData[FRAME_COMMAND_INDEX])
                {
                    ...
                    case FRAME_SHELL_DEBUG_START:
                        protocol_exit();
                        break;

                    case FRAME_SHELL_DEBUG_STOP:
                        uart_irq_callback_set(uart_console_dev, uart_recv_data_callback);
                        msendstart = 1;
                        break;
#endif
                    default:
...
static void txdata_process_thread()
{
    ...
    while (1)
    {
        ...
        {
            PRINTK("%x ", g_txData[i]);
            uart_poll_out(uart_console_dev, g_txData[i]);
        }
...
void protocol_sendstart()
{
    msendstart = 1;
}

void protocol_sendstop()
{
    msendstart = 0;
}
