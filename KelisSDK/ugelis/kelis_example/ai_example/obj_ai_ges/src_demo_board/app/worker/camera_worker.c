...
static void frame_ready(void *buf, void *arg)
{
    ...
    do_next_process(self, 0, &set);
}

int camera_worker_init(worker_t *self, void *param)
{

    dev_camera_t *dev = kmalloc(sizeof(dev_camera_t));
    dev->cam_cb.func = frame_ready;
    dev->cam_cb.arg  = (void *)self;
    dev->buf_w       = INPUT_WIDTH;
    dev->buf_h       = INPUT_HEIGHT;
    dev->mem_start   = 0;
    camera_init(dev);
    dev->buf_gray = kmalloc(INPUT_WIDTH * INPUT_HEIGHT);
    self->data = dev;
}
