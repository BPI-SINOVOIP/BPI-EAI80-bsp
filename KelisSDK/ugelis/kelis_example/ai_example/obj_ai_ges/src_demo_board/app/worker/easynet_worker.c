...
void ai_ges_detect(easynet_worker_t *easynet, ObjDetData *pobj_det, unsigned char *ops, unsigned short *inputbuf, unsigned short *outputbuf)
{
...
static void predict_process(worker_t *self, easynet_worker_t *easynet)
{
    while (1)
    {
        ...
        if (ret == 0)
        {
            ...
            ai_ges_detect(easynet, &gobj_det_data, (unsigned char *)easynet->ops, &ginputImage[0], &goutputImage[0]);
...
struct easynet_dev *ai_objectdetect_init(ObjDetData *pobj_det, unsigned char *ops, unsigned short *inputbuf, unsigned short *outputbuf)
{
    int i = 0;
    ...
    pobj_det->boxcnt = 0;
    pobj_det->show_demo_pic = 0;
    pobj_det->det_type =  DET_GEST;
    pobj_det->det_type_cpy = DET_GEST ;
...
static int easynet_worker_init(worker_t *self, void *param)
{
    ...
    easynet->dev = ai_objectdetect_init(&gobj_det_data, WEI_BASE, &ginputImage[0], &goutputImage[0]);
    self->data = easynet;
