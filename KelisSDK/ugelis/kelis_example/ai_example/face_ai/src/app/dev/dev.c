...
void ai_init_imp(uint32_t input_pixfmt, uint32_t output_pixfmt)
{
...
int ai_imp_scale(scaler_param_t *param)
{
...
#if 0
    printf("src->dst:(%x=>%x) wxh:(%dx%d)=>(%dx%d) fmt:(%d=>%d) used:%d\n",
           param->src,
           param->dst,
           param->src_w,
           param->src_h,
    ...
    //uint32_t input_pixfmt  = IMP_INPUT_RGB565;
    //uint32_t output_pixfmt = FMT_L8;//FMT_RGB888;
    ai_init_imp(param->src_fmt, param->dst_fmt);
...
{
    fb_convert_color(ltdc, src, dst, FMT_RGB565, FMT_L8, src_width, src_h);
    return 0;
}
