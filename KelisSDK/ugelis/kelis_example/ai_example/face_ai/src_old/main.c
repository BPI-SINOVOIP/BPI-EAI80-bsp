...
int parse_base(FaceAIData *fad, char *content_bytes)
{
    ...
    printf("magic:%s version:%s label_cnt:%d\n", magic, version, label_cnt);
...
int import_record(struct easynet_dev *dev, FaceAIData *fad, int backup_base)
{
    ...
    {
        printf("base_backup (%d/%d)\n", fad->base_backup.index, RECORD_BACKUP_CNT);
...
int delete_record(FaceAIData *fad, int label)
{
    ...
    printf("label [%d] has been deleted %d vectors\n", label, cnt);
    return cnt;
}
...
int calc_distance(FaceAIData *fad)
{
    ...
    if (fad->debug_print)
    {
        ...
        for (i = 0; i < buf_size; i++)
        {
            printf("%4d ", fad->dist.label[fad->dist.index[i]]);
        }
        printf("\n");

        printf("Corresponding number of occurrences:\n");
