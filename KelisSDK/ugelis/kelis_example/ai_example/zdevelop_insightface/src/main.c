...
#define BUFFERSIZE    (INPUT_WIDTH * INPUT_HEIGHT * 64) //         /*INPUT_WIDTH * INPUT_HEIGHT * 4*/
...
__aligned(32) unsigned short LayerBuffer[MAX_LAYER_BUFFER_NUM][BUFFERSIZE] _GENERIC_SECTION_ZI(mem_ai_data);
...
{
    obj_det_data.ex_buf.cam_buf_detected = malloc_ext(0, INPUT_WIDTH * INPUT_WIDTH * 3);
    ...
    obj_det_data.ex_buf.cam_buf_tmp = malloc_ext(0, INPUT_WIDTH * INPUT_WIDTH * 4); /*A, R, G ,B, it's layer 1, blending on layer 2*/

    printf("cam_buf_tmp : 0x%08x \n", obj_det_data.ex_buf.cam_buf_tmp);

    memset((void *)obj_det_data.ex_buf.cam_buf_tmp, 0x00, INPUT_WIDTH * INPUT_WIDTH * 4);
...
void body_detect(void)
{
    ...
    dev_cfg.layer_buffer[2] = &LayerBuffer[2][0];
    dev_cfg.layer_buffer[3] = &LayerBuffer[3][0];
    dev_cfg.layer_buffer[4] = &LayerBuffer[4][0];
    dev_cfg.layer_buffer[5] = &LayerBuffer[5][0];
    ...
    while (g_app_exit == 1)
    {
        ...
        if (obj_det_data.show_demo_pic)
        {
            ...
            memset(&LayerBuffer[4][0], 0x00, BUFFERSIZE * sizeof(unsigned short));
