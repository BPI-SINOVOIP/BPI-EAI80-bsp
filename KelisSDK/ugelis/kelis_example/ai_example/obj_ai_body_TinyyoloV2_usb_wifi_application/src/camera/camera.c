...
int camera_config(dcmi_frame_irq_callback_t cb)
{
    ...
    ret = dcmi_start_streaming(dcmi, VIN_MODE_SNAPSHOT /*VIN_MODE_CONTINUOUS VIN_MODE_SNAPSHOT*/);
...
int camera_jpeg_config()
{
    ...
    ret = dcmi_stop_streaming(dcmi);
    if (ret != 0)
    {
        return -ENOTSUP;
    }

    dcmi_ioctl(dcmi, VIDIOC_CONV_COLORSPACE, &csc_off);
    camera_set_power(camera_sensor, true);

    camera_set_fmt(camera_sensor, VIDEO_FMT_JPEG, MODE_720P_1280_720, FPS_15);//MODE_720P_1280_720 MODE_VGA_640_480 MODE_QSXGA_2592_1944
    ...
    fmt.pix_fmt = VIDEO_FMT_JPEG;
    fmt.width = v_fmt.width;
    fmt.height = v_fmt.height;
    dcmi_set_fmt_vid_cap(dcmi, &fmt);
    ...
    //The size of every frame is rect.width*rect.height
    reqbufs.count = 3;
    reqbufs.mem_start = obj_det_data.ex_buf.camera_jpeg_buf;//CAMERA_BUF_BASE;
    reqbufs.length = 0x28000 * 3;
    dcmi_ioctl(dcmi, VIDIOC_CREATE_BUFS, &reqbufs);
