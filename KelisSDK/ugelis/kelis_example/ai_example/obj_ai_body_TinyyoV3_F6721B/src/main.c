    ...
    do
    {
        ...
        if (ret == 0x06 || ret == 0x01)
        {
            if (ret == 0x06)
            {
                ...
                yolo_out_buffer[0] = dev->yolo_out_buffer[0];
                yolo_out_buffer[1] = dev->yolo_out_buffer[1];
                for (i = 0; i < MAX_SHORTCUT_NUM; i++)
                {
                    shortcut_buf[i] = dev->shortcut_buf[i];
                }

                dev = easynet_process_update(ops_data, pInputImage, pOutputImage);
                dev->predict_out = poutbuf;
                dev->yolo_out_buffer[0] = yolo_out_buffer[0];
                dev->yolo_out_buffer[1] = yolo_out_buffer[1];
                for (i = 0; i < MAX_SHORTCUT_NUM; i++)
                {
                    dev->shortcut_buf[i] = shortcut_buf[i];
                }
            }
            break;
        }
...
void body_detect(void)
{
    ...
    int mask_7x7[3] = {3, 4, 5};
    float biases_anchors7x7[10] = {5, 13,  15, 39,  37, 72,  46, 136,  90, 154, 171, 171}; // body detect

    int mask_14x14[3] = {0, 1, 2};
    float biases_anchors14x14[10] = {5, 13,  15, 39,  37, 72,  46, 136,  90, 154, 171, 171}; // body detect

    int nboxes;
    float thresh = 0.5;
    float nms_thresh = 0.7;  /*float nms_thresh = 0.40*/
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {
            layer[i].mask =  &mask_14x14[0];;
            layer[i].biases = &biases_anchors14x14[0];
            layer[i].w = 14;
            layer[i].h = 14;
        }
        ...
        layer[i].net_h = 224;
        layer[i].net_w = 224;
        layer[i].out_ch = 3;// l.out_ch; //num_box:5
        layer[i].classes = 1;
        layer[i].out_cnt = layer[i].w * layer[i].h * layer[i].out_ch * (layer[i].classes + 4 + 1);
        layer[i].post_type = YOLO;
        layer[i].output = (float *)malloc_ext(0, layer[i].out_cnt * sizeof(float));
        layer[i].softmax_tmp = (float *)malloc_ext(0, layer[i].out_ch * layer[i].classes * layer[i].w * layer[i].h * sizeof(float));
    }
    ...
    while (1)
    {
        ...
        obj_det_data.show_demo_pic = 1;   /*debug use*/
        if (obj_det_data.show_demo_pic)
        {
            nms_thresh = 0.40;
            /*for CNN computation*/
            dev->input = RecvBuffer;
            dev->network_type = TINYYOLOV3;
        ...
        dets = easynet_post_yolo(layer, &nboxes, thresh, nms_thresh,  &dev->yolo_out_buffer[0][0], &dev->yolo_out_buffer[1][0]);
