...
static void forward_yolo_layer(const layer_post l, void *rintput, float *routput);
...
static float overlap_version2(float x1, float x2, float x3, float x4)
{
    if (x3 < x1)
    {
        if (x4 < x1)
...
{
    float w = overlap_version2(a.x, a.w, b.x, b.w);
    float h = overlap_version2(a.y, a.h, b.y, b.h);
    if (w < 0 || h < 0)
    {
        return 0;
    }
...
float box_union(box a, box b, float intersect)
{
    float w1, h1, w2, h2;
    w1 = a.w - a.x;
    h1 = a.h - a.y;
...
{
    float intersect = box_intersection(a, b);
    return intersect / box_union(a, b, intersect);
}


typedef float (*box_iou_call)(box, box);
static void do_nms_sort(detection *dets, int total, int classes, float nms_thresh)
{
    int i, j, k;
    box_iou_call iou_call;;
    k = total - 1;
    ...
    for (k = 0; k < classes; ++k)
    {
        ...
        for (i = 0; i < total; ++i)
        {
            ...
            {
                box b = dets[j].bbox;
                float iou = iou_call(a, b);
                if (iou_call(a, b) > nms_thresh)
                {
...
static box get_yolo_box(float *x, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride)
{
    ...
    {
        b.w = exp_f(x[index + 2 * stride]) * biases[2 * n]   / lw;
        b.h = exp_f(x[index + 3 * stride]) * biases[2 * n + 1] / lh;
    }
...
void correct_yolo_boxes(detection *dets, int n)
{
    int i;
    float x, y, w, h;
    for (i = 0; i < n; ++i)
    ...
    for (i = 0; i < l.w * l.h; ++i)
    {
        ...
        for (n = 0; n < l.out_ch; ++n)
        {
            ...
            for (j = 0; j < l.classes; ++j)
            {
                ...
                //printf("dets[%d].bbox.x = %f\n\n", count, dets[count].bbox.x);
            }
...
static detection *make_network_boxes(layer_post layer0, layer_post layer1, float thresh, int *num)
{
    ...
    if (nboxes == 0)
    {
        det = (detection *)malloc_ext(0, 1 * sizeof(detection));
        for (i = 0; i < 1; ++i)
        {
    ...
    {
        det = (detection *)malloc_ext(0, nboxes * sizeof(detection));
        for (i = 0; i < nboxes; ++i)
        {
            det[i].prob = (float *)malloc_ext(0, layer0.classes * sizeof(float));
        }
...
detection *easynet_post_yolo(layer_post l[2], int *nboxes, float thresh, float nms_thresh, void *out, float *foutput)
{
    int i, k;
    int ch, row, col, off;
    detection *dets = NULL;
    *nboxes = 0;
    ...
    forward_yolo_layer(l[0], out, l[0].output);
...
static float max_cpu(float *in, int n)
{
    int i;
...
static void softmax_cpu(float *in, int n, float temp, float *output, int stride)
{
    int i;
    float sum = 0;
    float largest = -FLT_MAX;
    ...
    for (i = 0; i < n; ++i)
    {
        float e = exp_f(in[i * stride] / temp - largest / temp);
        sum += e;
        output[i * stride] = e;
    }
    for (i = 0; i < n; ++i)
...
static void forward_yolo_layer(const layer_post l, void *rintput, float *routput)
{
    ...
    int row = 0;
    int col = 0;
    int ch_num = (l.out_ch * (l.classes + 4 + 1));
    int row_num = l.h;
    int col_num = l.w;
    int off = 0;
    short int *out = rintput;

    for (ch = 0; ch < ch_num; ++ch)
    {
        for (row = 0; row < row_num; ++row)
        {
            off = (row + row_num * ch) * col_num;
            for (col = 0; col < col_num; ++col)
            {
                out_val = out[off + col];
                l.output[off + col] = ((float)out_val) / 256.0;
            }
        }
    }
    ...
    for (n = 0; n < l.out_ch; ++n)
    {
        index = entry_index(l,  n * l.w * l.h, ENTRY_X);
        activate_array(l.output + index, 2 * l.w * l.h, LOGISTIC);
        index = entry_index(l,  n * l.w * l.h, ENTRY_CONFIDENCE);
        activate_array(l.output + index, 1 * l.w * l.h, LOGISTIC);
    }

    softmax_tmp = malloc_ext(0, l.out_ch * l.classes * l.w * l.h * sizeof(float));
    for (n = 0; n < l.out_ch; ++n)
    {
        index = entry_index(l, n * l.w * l.h, ENTRY_CLASS);
        memcpy(softmax_tmp + n * l.classes * l.w * l.h, l.output + index, l.classes * l.w * l.h * sizeof(float));
    }
    max = max_cpu(softmax_tmp, l.out_ch * l.classes * l.w * l.h);

    for (i = 0; i < l.w * l.h; i++)
    {
        for (n = 0; n < l.out_ch; ++n)
        {
            index = entry_index(l, n * l.w * l.h + i, ENTRY_CLASS);
            softmax_cpu(l.output + index, l.classes, 1, l.output + index, 1);

        }
    }

    free_ext(0, softmax_tmp);
}
...
inline void activate_array(float *x, const int n, const ACTIVATION a)
{
    int i;
    for (i = 0; i < n; ++i)
    {
...
inline void softmax_ext(float *input, int n, float temp, int stride, float *output)
{
    int i;
    float sum = 0;
...
inline void softmax_cpu_ext(float *input, int n, int batch, int batch_offset, int groups, int group_offset, int stride, float temp, float *output)
{
    int g, b;
    for (b = 0; b < batch; ++b)
    {
