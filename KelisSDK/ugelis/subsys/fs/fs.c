...
int fs_ioctl(struct fs_file_t *zfp, int cmd, void *data)
{
    int rc = -EINVAL;

    if (zfp->mp->fs->ioctl != NULL)
    {
        rc = zfp->mp->fs->ioctl(zfp, cmd, data);
        if (rc < 0)
        {
            SYS_LOG_ERR("file seek error (%d)", rc);
        }
...
int fs_mkfile(const char *abs_path, void *data)
{
    struct fs_mount_t *mp;
    int rc = -EINVAL;
    ...
    if (mp->fs->mkfile != NULL)
    {
        rc = mp->fs->mkfile(mp, abs_path, data);
        if (rc < 0)
        {
            SYS_LOG_ERR("failed to create directory (%d)", rc);
        }
