 ...
 *  STATIC PROTOTYPES
 **********************/
void tri_draw_flat(const edgui_point_t * points, const edgui_area_t * mask, const edgui_style_t * style, edgui_opa_t opa);
void tri_draw_tall(const edgui_point_t * points, const edgui_area_t * mask, const edgui_style_t * style, edgui_opa_t opa);
static void point_swap(edgui_point_t * p1, edgui_point_t * p2);
...
void edgui_draw_triangle(const edgui_point_t * points, const edgui_area_t * mask, const edgui_style_t * style, edgui_opa_t opa_scale)
{
    ...
    edgui_coord_t x_min = EDGUI_MATH_MIN(EDGUI_MATH_MIN(points[0].x, points[1].x), points[2].x);
    edgui_coord_t x_max = EDGUI_MATH_MAX(EDGUI_MATH_MAX(points[0].x, points[1].x), points[2].x);
    edgui_coord_t y_min = EDGUI_MATH_MIN(EDGUI_MATH_MIN(points[0].y, points[1].y), points[2].y);
    edgui_coord_t y_max = EDGUI_MATH_MAX(EDGUI_MATH_MAX(points[0].y, points[1].y), points[2].y);
                     ...
                     edgui_opa_t opa_scale)
{
    ...
    for(i = 0; i < point_cnt - 1; i++) {
        ...
        edgui_draw_triangle(tri, mask, style, opa_scale);
    }
}
...
void tri_draw_flat(const edgui_point_t * points, const edgui_area_t * mask, const edgui_style_t * style, edgui_opa_t opa)
{
    ...
    /*Draw the triangle*/
    edgui_point_t edge1;
    edgui_coord_t dx1  = EDGUI_MATH_ABS(tri[0].x - tri[1].x);
    edgui_coord_t sx1  = tri[0].x < tri[1].x ? 1 : -1;
    edgui_coord_t dy1  = EDGUI_MATH_ABS(tri[0].y - tri[1].y);
    edgui_coord_t sy1  = tri[0].y < tri[1].y ? 1 : -1;
    edgui_coord_t err1 = (dx1 > dy1 ? dx1 : -dy1) / 2;
    edgui_coord_t err_tmp1;

    edgui_point_t edge2;
    edgui_coord_t dx2  = EDGUI_MATH_ABS(tri[0].x - tri[2].x);
    edgui_coord_t sx2  = tri[0].x < tri[2].x ? 1 : -1;
    edgui_coord_t dy2  = EDGUI_MATH_ABS(tri[0].y - tri[2].y);
    edgui_coord_t sy2  = tri[0].y < tri[2].y ? 1 : -1;
    edgui_coord_t err2 = (dx1 > dy2 ? dx2 : -dy2) / 2;
    edgui_coord_t err_tmp2;

    edgui_coord_t y1_tmp;
    edgui_coord_t y2_tmp;
    ...
    while(1) {
        ...
        draw_area.x1 = EDGUI_MATH_MIN(act_area.x1, act_area.x2) + 1;
        draw_area.x2 = EDGUI_MATH_MAX(act_area.x1, act_area.x2);
        draw_area.y1 = EDGUI_MATH_MIN(act_area.y1, act_area.y2) - 1;
        draw_area.y2 = EDGUI_MATH_MAX(act_area.y1, act_area.y2) - 1;

        edgui_draw_fill(&draw_area, mask, style->body.main_color, opa);
        ...
        do {
            if(edge1.x == tri[1].x && edge1.y == tri[1].y) {
                ...
                dx1  = EDGUI_MATH_ABS(tri[1].x - tri[2].x);
                sx1  = tri[1].x < tri[2].x ? 1 : -1;
                dy1  = EDGUI_MATH_ABS(tri[1].y - tri[2].y);
...
void tri_draw_tall(const edgui_point_t * points, const edgui_area_t * mask, const edgui_style_t * style, edgui_opa_t opa)
{
    ...
    edgui_coord_t x1_tmp;
    edgui_coord_t x2_tmp;
