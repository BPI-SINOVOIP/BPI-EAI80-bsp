...
typedef struct
{
    ...
    edgui_point_t p_act;
    edgui_coord_t dx;
    edgui_coord_t sx; /*-1: x1 < x2; 1: x2 >= x1*/
    edgui_coord_t dy;
    edgui_coord_t sy; /*-1: y1 < y2; 1: y2 >= y1*/
    edgui_coord_t err;
    edgui_coord_t e2;
...
{
    edgui_coord_t width;
    edgui_coord_t width_1;
    edgui_coord_t width_half;
} line_width_t;

/**********************
 *  STATIC PROTOTYPES
 **********************/
static void line_draw_hor(line_draw_t * main_line, const edgui_area_t * mask, const edgui_style_t * style,
                          edgui_opa_t opa_scale);
static void line_draw_ver(line_draw_t * main_line, const edgui_area_t * mask, const edgui_style_t * style,
                          edgui_opa_t opa_scale);
static void line_draw_skew(line_draw_t * main_line, bool dir_ori, const edgui_area_t * mask, const edgui_style_t * style,
                           edgui_opa_t opa_scale);
static void line_init(line_draw_t * line, const edgui_point_t * p1, const edgui_point_t * p2);
                  ...
                  const edgui_style_t * style, edgui_opa_t opa_scale)
{
    ...
    if(EDGUI_MATH_ABS(point1->x - point2->x) > EDGUI_MATH_ABS(point1->y - point2->y)) {
        ...
        if(aa) {
            ...
            if(main_line.hor) {
                if(main_line.p1.y < main_line.p2.y) {
                    ...
                    main_line.sy = EDGUI_MATH_ABS(main_line.sy); /*The sign can change if the line becomes horizontal*/
                ...
                if(main_line.p1.x < main_line.p2.x) {
                    ...
                    line_init(&main_line, &p1, &p_tmp);
                    main_line.sx = EDGUI_MATH_ABS(main_line.sx); /*The sign can change if the line becomes vertical*/
                } else if(main_line.p1.x > main_line.p2.x) {
                    dir_ori = false;
                    p_tmp.x = main_line.p2.x + 1;
                    p_tmp.y = main_line.p2.y;
...
static void line_draw_hor(line_draw_t * main_line, const edgui_area_t * mask, const edgui_style_t * style, edgui_opa_t opa_scale)
{
    edgui_coord_t width      = style->line.width - 1;
    edgui_coord_t width_half = width >> 1;
    edgui_coord_t width_1    = width & 0x1;
    edgui_opa_t opa = opa_scale == EDGUI_OPA_COVER ? style->line.opa : (uint16_t)((uint16_t)style->line.opa * opa_scale) >> 8;

    edgui_area_t act_area;
    ...
    edgui_area_t draw_area;
    draw_area.x1 = EDGUI_MATH_MIN(act_area.x1, act_area.x2);
    draw_area.x2 = EDGUI_MATH_MAX(act_area.x1, act_area.x2);
    draw_area.y1 = EDGUI_MATH_MIN(act_area.y1, act_area.y2);
    draw_area.y2 = EDGUI_MATH_MAX(act_area.y1, act_area.y2);
    edgui_draw_fill(&draw_area, mask, style->line.color, opa);
}

static void line_draw_ver(line_draw_t * main_line, const edgui_area_t * mask, const edgui_style_t * style, edgui_opa_t opa_scale)
{
    edgui_coord_t width      = style->line.width - 1;
    edgui_coord_t width_half = width >> 1;
    edgui_coord_t width_1    = width & 0x1;
                           ...
                           edgui_opa_t opa_scale)
{
    ...
    bool aa = edgui_disp_get_antialiasing(edgui_refr_get_disp_refreshing());
#endif
    edgui_point_t vect_main, vect_norm;
    ...
    edgui_coord_t pattern_size = width * 2;
    edgui_point_t * pattern = edgui_draw_get_buf(pattern_size * sizeof(edgui_point_t));
    edgui_coord_t i = 0;

    /*Create a perpendicular pattern (a small line)*/
    if(width != 0) {
        line_draw_t pattern_line;
        edgui_point_t p0 = {0, 0};
        ...
        for(i = 0; i < (edgui_coord_t)pattern_size - 1; i++) {
...
#if EDGUI_ANTIALIAS
    edgui_coord_t aa_last_corner;
    edgui_coord_t width_safe = width;
    if(aa) {
    ...
    edgui_coord_t x_center_ofs = 0;
    edgui_coord_t y_center_ofs = 0;
            ...
            if(i != 0) {
                if(main_line->hor) {
                    if(pattern[i - 1].x != pattern[i].x) {
                        edgui_coord_t seg_w = pattern[i].y - pattern[aa_last_corner].y;
                        if(main_line->sy < 0) {
                            edgui_draw_aa_ver_seg(main_line->p1.x + pattern[aa_last_corner].x - 1,
                                               main_line->p1.y + pattern[aa_last_corner].y + seg_w + 1, seg_w, mask,
                                               style->line.color, opa);

                            edgui_draw_aa_ver_seg(main_line->p2.x + pattern[aa_last_corner].x + 1,
                    ...
                    if(pattern[i - 1].y != pattern[i].y) {
                        edgui_coord_t seg_w = pattern[i].x - pattern[aa_last_corner].x;
                        if(main_line->sx < 0) {
                            edgui_draw_aa_hor_seg(main_line->p1.x + pattern[aa_last_corner].x + seg_w + 1,
                                               main_line->p1.y + pattern[aa_last_corner].y - 1, seg_w, mask,
                                               style->line.color, opa);

                            edgui_draw_aa_hor_seg(main_line->p2.x + pattern[aa_last_corner].x + seg_w + 1,
                                               main_line->p2.y + pattern[aa_last_corner].y + 1, -seg_w, mask,
                                               style->line.color, opa);
                        } else {
                            edgui_draw_aa_hor_seg(main_line->p1.x + pattern[aa_last_corner].x,
                                               main_line->p1.y + pattern[aa_last_corner].y - 1, seg_w, mask,
                                               style->line.color, opa);

                            edgui_draw_aa_hor_seg(main_line->p2.x + pattern[aa_last_corner].x,
                                               main_line->p2.y + pattern[aa_last_corner].y + 1, -seg_w, mask,
                                               style->line.color, opa);
                        }
                        aa_last_corner = i;
    ...
    if(width != 0 && aa) { /*Due to rounding error with very thin lines it looks ugly*/
        if(main_line->hor) {
            edgui_coord_t seg_w = pattern[width_safe - 1].y - pattern[aa_last_corner].y;
            if(main_line->sy < 0) {
                edgui_draw_aa_ver_seg(main_line->p1.x + pattern[aa_last_corner].x - 1,
            ...
            edgui_coord_t seg_w = pattern[width_safe - 1].x - pattern[aa_last_corner].x;
            if(main_line->sx < 0) {
                edgui_draw_aa_hor_seg(main_line->p1.x + pattern[aa_last_corner].x + seg_w,
                                   main_line->p1.y + pattern[aa_last_corner].y - 1, seg_w + main_line->sx, mask,
                                   style->line.color, opa);

                edgui_draw_aa_hor_seg(main_line->p2.x + pattern[aa_last_corner].x + seg_w,
    ...
    edgui_coord_t aa_shift1 = 0;
    edgui_coord_t aa_shift2 = 0;
    if(aa) {
    ...
    if(main_line->hor) {
        while(line_next_y(main_line)) {
            for(i = 0; i < width; i++) {
                ...
                if(i != 0 && pattern[i].x != pattern[i - 1].x && !first_run) {
                    edgui_draw_px(draw_area.x1, draw_area.y1 - main_line->sy, mask, style->line.color, opa);
                }
            }

#if EDGUI_ANTIALIAS
            if(aa) {
                edgui_draw_aa_hor_seg(prev_p.x + pattern[0].x, prev_p.y + pattern[0].y - aa_shift1,
                                   -(main_line->p_act.x - prev_p.x), mask, style->line.color, opa);
                edgui_draw_aa_hor_seg(prev_p.x + pattern[width_safe - 1].x,
        ...
        if(aa) {
            ...
            edgui_draw_aa_hor_seg(prev_p.x + pattern[width_safe - 1].x, prev_p.y + pattern[width_safe - 1].y + aa_shift2,
                               main_line->p_act.x - prev_p.x + 1, mask, style->line.color, opa);
        }
#endif
    }
    /*Rather a vertical line*/
        ...
        while(line_next_x(main_line)) {
            for(i = 0; i < width; i++) {
                ...
                if(i != 0 && pattern[i].y != pattern[i - 1].y && !first_run) {
                    edgui_draw_px(draw_area.x1 - main_line->sx, draw_area.y1, mask, style->line.color, opa);
                }
            }

#if EDGUI_ANTIALIAS
            if(aa) {
                edgui_draw_aa_ver_seg(prev_p.x + pattern[0].x - aa_shift1, prev_p.y + pattern[0].y,
                                   -(main_line->p_act.y - prev_p.y), mask, style->line.color, opa);
                edgui_draw_aa_ver_seg(prev_p.x + pattern[width_safe - 1].x + aa_shift2,
        ...
        if(aa) {
            ...
            edgui_draw_aa_ver_seg(prev_p.x + pattern[width_safe - 1].x + aa_shift2, prev_p.y + pattern[width_safe - 1].y,
                               main_line->p_act.y - prev_p.y + 1, mask, style->line.color, opa);
        }
#endif
    }
...
static void line_init(line_draw_t * line, const edgui_point_t * p1, const edgui_point_t * p2)
{
    ...
    line->dx  = EDGUI_MATH_ABS(line->p2.x - line->p1.x);
    line->sx  = line->p1.x < line->p2.x ? 1 : -1;
    line->dy  = EDGUI_MATH_ABS(line->p2.y - line->p1.y);
...
static bool line_next_y(line_draw_t * line)
{
    edgui_coord_t last_y = line->p_act.y;
...
static bool line_next_x(line_draw_t * line)
{
    edgui_coord_t last_x = line->p_act.x;
