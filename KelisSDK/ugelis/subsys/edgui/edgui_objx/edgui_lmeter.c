 ...
 *      DEFINES
 *********************/
#define EDGUI_LMETER_LINE_UPSCALE 5 /*2^x upscale of line to make rounding*/
#define EDGUI_LMETER_LINE_UPSCALE_MASK ((1 << EDGUI_LMETER_LINE_UPSCALE) - 1)
 ...
 *  STATIC PROTOTYPES
 **********************/
static bool edgui_lmeter_design(edgui_obj_t * lmeter, const edgui_area_t * mask, edgui_design_mode_t mode);
static edgui_res_t edgui_lmeter_signal(edgui_obj_t * lmeter, edgui_signal_t sign, void * param);
static edgui_coord_t edgui_lmeter_coord_round(int32_t x);
...
{
    EDGUI_LOG_TRACE("line meter create started");

    /*Create the ancestor of line meter*/
    edgui_obj_t * new_lmeter = edgui_obj_create(par, copy);
    edgui_mem_assert(new_lmeter);
    if(new_lmeter == NULL) return NULL;

    if(ancestor_signal == NULL) ancestor_signal = edgui_obj_get_signal_cb(new_lmeter);

    /*Allocate the line meter type specific extended data*/
    edgui_lmeter_ext_t * ext = edgui_obj_allocate_ext_attr(new_lmeter, sizeof(edgui_lmeter_ext_t));
    ...
    edgui_obj_set_signal_cb(new_lmeter, edgui_lmeter_signal);
    edgui_obj_set_design_cb(new_lmeter, edgui_lmeter_design);

    /*Init the new line meter line meter*/
    if(copy == NULL) {
        edgui_obj_set_size(new_lmeter, EDGUI_DPI, EDGUI_DPI);

        /*Set the default styles*/
        edgui_theme_t * th = edgui_theme_get_current();
        if(th) {
            edgui_lmeter_set_style(new_lmeter, EDGUI_LMETER_STYLE_MAIN, th->style.lmeter);
        } else {
            edgui_lmeter_set_style(new_lmeter, EDGUI_LMETER_STYLE_MAIN, &edgui_style_pretty_color);
        }
    }
    /*Copy an existing line meter*/
    else {
        edgui_lmeter_ext_t * copy_ext = edgui_obj_get_ext_attr(copy);
        ...
        edgui_obj_refresh_style(new_lmeter);
    }

    EDGUI_LOG_INFO("line meter created");
...
{
    edgui_lmeter_ext_t * ext = edgui_obj_get_ext_attr(lmeter);
...
void edgui_lmeter_set_range(edgui_obj_t * lmeter, int16_t min, int16_t max)
{
    ...
    if(ext->cur_value > max) {
        ext->cur_value = max;
        edgui_lmeter_set_value(lmeter, ext->cur_value);
    }
...
static bool edgui_lmeter_design(edgui_obj_t * lmeter, const edgui_area_t * mask, edgui_design_mode_t mode)
{
    ...
    /*Draw the object*/
        ...
        edgui_lmeter_ext_t * ext    = edgui_obj_get_ext_attr(lmeter);
        const edgui_style_t * style = edgui_obj_get_style(lmeter);
        edgui_opa_t opa_scale       = edgui_obj_get_opa_scale(lmeter);
        edgui_style_t style_tmp;
        edgui_style_copy(&style_tmp, style);

#if EDGUI_USE_GROUP
        edgui_group_t * g = edgui_obj_get_group(lmeter);
        if(edgui_group_get_focused(g) == lmeter) {
            style_tmp.line.width += 1;
        }
#endif

        edgui_coord_t r_out = edgui_obj_get_width(lmeter) / 2;
        edgui_coord_t r_in  = r_out - style->body.padding.left;
        if(r_in < 1) r_in = 1;

        edgui_coord_t x_ofs  = edgui_obj_get_width(lmeter) / 2 + lmeter->coords.x1;
        edgui_coord_t y_ofs  = edgui_obj_get_height(lmeter) / 2 + lmeter->coords.y1;
        ...
        r_out = r_out << EDGUI_LMETER_LINE_UPSCALE;
        r_in  = r_in << EDGUI_LMETER_LINE_UPSCALE;

        for(i = 0; i < ext->line_cnt; i++) {
            /*Calculate the position a scale label*/
            int16_t angle = (i * ext->scale_angle) / (ext->line_cnt - 1) + angle_ofs;

            edgui_coord_t y_out = (int32_t)((int32_t)edgui_trigo_sin(angle) * r_out) >> EDGUI_TRIGO_SHIFT;
            edgui_coord_t x_out = (int32_t)((int32_t)edgui_trigo_sin(angle + 90) * r_out) >> EDGUI_TRIGO_SHIFT;
            edgui_coord_t y_in  = (int32_t)((int32_t)edgui_trigo_sin(angle) * r_in) >> EDGUI_TRIGO_SHIFT;
            edgui_coord_t x_in  = (int32_t)((int32_t)edgui_trigo_sin(angle + 90) * r_in) >> EDGUI_TRIGO_SHIFT;

            /*Rounding*/
            x_out = edgui_lmeter_coord_round(x_out);
            x_in  = edgui_lmeter_coord_round(x_in);
            y_out = edgui_lmeter_coord_round(y_out);
            y_in  = edgui_lmeter_coord_round(y_in);
            ...
            if(i >= level)
                ...
                style_tmp.line.color =
                    edgui_color_mix(style->body.grad_color, style->body.main_color, (255 * i) / ext->line_cnt);
            }

            edgui_draw_line(&p1, &p2, mask, &style_tmp, opa_scale);
        }
...
static edgui_res_t edgui_lmeter_signal(edgui_obj_t * lmeter, edgui_signal_t sign, void * param)
{
    ...
    if(sign == EDGUI_SIGNAL_CLEANUP) {
        ...
        edgui_obj_refresh_ext_draw_pad(lmeter);
    } else if(sign == EDGUI_SIGNAL_REFR_EXT_DRAW_PAD) {
        const edgui_style_t * style = edgui_lmeter_get_style(lmeter, EDGUI_LMETER_STYLE_MAIN);
        lmeter->ext_draw_pad     = EDGUI_MATH_MAX(lmeter->ext_draw_pad, style->line.width);
...
static edgui_coord_t edgui_lmeter_coord_round(int32_t x)
{
#if EDGUI_LMETER_LINE_UPSCALE > 0
    ...
    x = (x >> EDGUI_LMETER_LINE_UPSCALE) + ((x & EDGUI_LMETER_LINE_UPSCALE_MASK) >> (EDGUI_LMETER_LINE_UPSCALE - 1));
