...
#ifndef EDGUI_FONT_FMT_TXT_H
#define EDGUI_FONT_FMT_TXT_H
...
}edgui_font_fmt_txt_glyph_dsc_t;


/** Format of font character map. */
enum {
    EDGUI_FONT_FMT_TXT_CMAP_FORMAT0_TINY,
    EDGUI_FONT_FMT_TXT_CMAP_FORMAT0_FULL,
    EDGUI_FONT_FMT_TXT_CMAP_SPARSE_TINY,
    EDGUI_FONT_FMT_TXT_CMAP_SPARSE_FULL,
};

typedef uint8_t edgui_font_fmt_txt_cmap_type_t;


/* Map codepoints to a `glyph_dsc`s
 * Several formats are supported to optimize memory usage
 * See https://github.com/edgui/edgui_font_conv/blob/master/doc/font_spec.md
 */
typedef struct {
    ...
    According the specification there are 4 formats:
        https://github.com/edgui/edgui_font_conv/blob/master/doc/font_spec.md
    ...
    */
    ...
    /** if(type == EDGUI_FONT_FMT_TXT_CMAP_FORMAT0_...) it's `uint8_t *`
     * if(type == EDGUI_FONT_FMT_TXT_CMAP_SPARSE_...)  it's `uint16_t *`
     */
    ...
    edgui_font_fmt_txt_cmap_type_t type   :2;
}edgui_font_fmt_txt_cmap_t;

/** A simple mapping of kern values from pairs*/
typedef struct {
    /*To get a kern value of two code points:
       1. Get the `glyph_id_left` and `glyph_id_right` from `edgui_font_fmt_txt_cmap_t
...
}edgui_font_fmt_txt_kern_pair_t;
...
}edgui_font_fmt_txt_kern_classes_t;


/** Bitmap formats*/
typedef enum {
    EDGUI_FONT_FMT_TXT_PLAIN      = 0,
    EDGUI_FONT_FMT_TXT_COMPRESSED = 1,
}edgui_font_fmt_txt_bitmap_format_t;
...
typedef struct {
    ...
    /*Describe the glyphs*/
    const edgui_font_fmt_txt_glyph_dsc_t * glyph_dsc;

    /* Map the glyphs to Unicode characters.
     * Array of `edgui_font_cmap_fmt_txt_t` variables*/
    const edgui_font_fmt_txt_cmap_t * cmaps;

    /* Store kerning values.
     * Can be  `edgui_font_fmt_txt_kern_pair_t *  or `edgui_font_kern_classes_fmt_txt_t *`
     ...
     * from `edgui_font_fmt_txt_bitmap_format_t`
     */
 ...
 */
const uint8_t * edgui_font_get_bitmap_fmt_txt(const edgui_font_t * font, uint32_t letter);
 ...
 */
bool edgui_font_get_glyph_dsc_fmt_txt(const edgui_font_t * font, edgui_font_glyph_dsc_t * dsc_out, uint32_t unicode_letter, uint32_t unicode_letter_next);
