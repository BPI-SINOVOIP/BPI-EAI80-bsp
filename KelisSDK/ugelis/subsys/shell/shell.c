...
#else
extern unsigned char Load$$SHELL_MODULE_REGION$$Base;
extern unsigned char Image$$SHELL_MODULE_REGION$$Base;
extern unsigned char Image$$SHELL_MODULE_REGION$$Length;
extern unsigned char Load$$SHELL_MODULE_REGION$$Length;
extern unsigned char Image$$SHELL_MODULE_REGION$$Limit;

extern unsigned char Load$$SHELL_CMD_REGION$$Base;
extern unsigned char Image$$SHELL_CMD_REGION$$Base;
extern unsigned char Image$$SHELL_CMD_REGION$$Length;
extern unsigned char Load$$SHELL_CMD_REGION$$Length;
extern unsigned char Image$$SHELL_CMD_REGION$$Limit;

struct shell_module *__shell_module_start = (struct shell_module *)&Image$$SHELL_MODULE_REGION$$Base;
struct shell_module *__shell_module_end = (struct shell_module *)&Image$$SHELL_MODULE_REGION$$Limit;

struct shell_cmd *__shell_cmd_start = (struct shell_cmd *)&Image$$SHELL_CMD_REGION$$Base;
struct shell_cmd *__shell_cmd_end = (struct shell_cmd *)&Image$$SHELL_CMD_REGION$$Limit;
#endif
...
static void shell(void *p1, void *p2, void *p3)
{
	...
	printk("uGelis Shell, uGelis version: %s\n", KERNEL_VERSION_STRING);
...
{
#ifdef __USE_KEIL_
	__shell_module_start = (struct shell_module *)&Image$$SHELL_MODULE_REGION$$Base;
	__shell_module_end = (struct shell_module *)((uint32_t)&Image$$SHELL_MODULE_REGION$$Base + (uint32_t)&Load$$SHELL_MODULE_REGION$$Length);

	__shell_cmd_start = (struct shell_cmd *)&Image$$SHELL_CMD_REGION$$Base;
	__shell_cmd_end = (struct shell_cmd *)((uint32_t)&Image$$SHELL_CMD_REGION$$Base + (uint32_t)&Load$$SHELL_CMD_REGION$$Length);
#endif
