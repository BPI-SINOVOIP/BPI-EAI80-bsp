...
### Example to compile OpenAMP for uGelis
You can compile OpenAMP library for uGelis.
As OpenAMP uses libmetal, please refer to libmetal README to build libmetal
for uGelis before building OpenAMP library for uGelis.
As uGelis uses CMake, we build OpenAMP library as a target of uGelis CMake
project. Here is how to build libmetal for uGelis:
```
    $ export ZEPHRY_GCC_VARIANT=ugelis
    $ export ZEPHRY_SDK_INSTALL_DIR=<where uGelis SDK is installed>
    $ source <git_clone_ugelis_project_source_root>/ugelis-env.sh

    $ cmake <OpenAMP_source_root> \
      -DWITH_UGELIS=on -DBOARD=qemu_cortex_m3 \
      -DCMAKE_INCLUDE_PATH="<libmetal_ugelis_build_dir>/lib/include" \
      -DCMAKE_LIBRARY_PATH="<libmetal_ugelis_build_dir>/lib" \
    $ make VERBOSE=1 all
```
