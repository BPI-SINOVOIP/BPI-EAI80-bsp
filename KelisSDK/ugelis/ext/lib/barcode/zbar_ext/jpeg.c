...
{
    j_decompress_ptr cinfo = zbar_calloc(1, sizeof(struct jpeg_decompress_struct));
    if(!cinfo)
        return(NULL);

    errenv_t *jerr = zbar_calloc(1, sizeof(errenv_t));
    if(!jerr) {
        zbar_free(cinfo);
        return(NULL);
    }
                              ...
                              const zbar_format_def_t *srcfmt)
{
    ...
    /* setup input image */
    if(!cinfo->src) {
        cinfo->src = zbar_calloc(1, sizeof(zbar_src_mgr_t));
