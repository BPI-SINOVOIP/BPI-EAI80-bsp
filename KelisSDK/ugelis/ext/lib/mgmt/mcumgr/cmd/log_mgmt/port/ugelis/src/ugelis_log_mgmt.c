...
struct ugelis_log_mgmt_walk_arg {
...
static int
ugelis_log_mgmt_walk_cb(struct mdlog *log, struct mdlog_offset *log_offset,
                        const void *desciptor, uint16_t len)
{
    struct ugelis_log_mgmt_walk_arg *ugelis_log_mgmt_walk_arg;
    struct log_mgmt_entry entry;
    struct mdlog_entry_hdr ueh;
    int read_len;
    int rc;

    ugelis_log_mgmt_walk_arg = log_offset->lo_arg;
    ...
    rc = mdlog_read(log, desciptor, ugelis_log_mgmt_walk_arg->body, sizeof ueh,
                    read_len);
    if (rc < 0) {
        return MGMT_ERR_EUNKNOWN;
    }
    ...
    entry.len = rc;
    entry.data = ugelis_log_mgmt_walk_arg->body;

    return ugelis_log_mgmt_walk_arg->cb(&entry, ugelis_log_mgmt_walk_arg->arg);
}
...
{
    struct ugelis_log_mgmt_walk_arg walk_arg;
    struct mdlog_offset offset;
    struct mdlog *mdlog;

    walk_arg = (struct ugelis_log_mgmt_walk_arg) {
        .cb = cb,
        .arg = arg,
    };
    ...
    if (strcmp(mdlog->l_name, log_name) == 0) {
        ...
        return mdlog_walk(mdlog, ugelis_log_mgmt_walk_cb, &offset);
    }
