...
void _impl_k_queue_init(struct k_queue *queue)
{
	...
	_waitq_init(&queue->wait_u.wait_q);
#if defined(CONFIG_POLL)
	sys_dlist_init(&queue->wait_u.poll_events);
#endif
...
{
	_handle_obj_poll_events(&queue->wait_u.poll_events, state);
}
#endif
...
void _impl_k_queue_cancel_wait(struct k_queue *queue)
{
	...
	first_pending_thread = _unpend_first_thread(&queue->wait_u.wait_q);
...
void k_queue_append_list(struct k_queue *queue, void *head, void *tail)
{
	...
	while (head && ((thread = _unpend_first_thread(&queue->wait_u.wait_q)))) {
...
#else
	int ret = _pend_current_thread(key, &queue->wait_u.wait_q, timeout);
