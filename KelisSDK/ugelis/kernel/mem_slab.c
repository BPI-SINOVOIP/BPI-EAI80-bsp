...
#ifdef __USE_KEIL_
extern unsigned char Image$$KMEM_SLAB_REGION$$Base;
extern unsigned char Image$$KMEM_SLAB_REGION$$Length;

/* Linker-defined symbols bound the static pool structs */
struct k_mem_slab*_k_mem_slab_list_start; 
struct k_mem_slab* _k_mem_slab_list_end;
#else
...
#ifdef __USE_KEIL_
	_k_mem_slab_list_start = (struct k_mem_pool*)&Image$$KMEM_SLAB_REGION$$Base;
	_k_mem_slab_list_end = (struct k_mem_pool*)((uint32_t)&Image$$KMEM_SLAB_REGION$$Base + (uint32_t)&Image$$KMEM_SLAB_REGION$$Length);
#endif	
