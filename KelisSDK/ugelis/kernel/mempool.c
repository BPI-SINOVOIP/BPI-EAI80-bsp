...
#ifdef __USE_KEIL_
extern unsigned char Image$$KMEM_POOL_REGION$$Base;
extern unsigned char Image$$KMEM_POOL_REGION$$Length;

/* Linker-defined symbols bound the static pool structs */
struct k_mem_pool*_k_mem_pool_list_start; 
struct k_mem_pool* _k_mem_pool_list_end;
#else
/* Linker-defined symbols bound the static pool structs */
...
#ifdef __USE_KEIL_	
	_k_mem_pool_list_start = (struct k_mem_pool*)&Image$$KMEM_POOL_REGION$$Base;
	_k_mem_pool_list_end = (struct k_mem_pool*)(((uint32_t)&Image$$KMEM_POOL_REGION$$Base) + ((uint32_t)&Image$$KMEM_POOL_REGION$$Length));
#endif
...
#else
	size = size + sizeof(struct k_mem_block_id);
#endif	
