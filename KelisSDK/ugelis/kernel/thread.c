...
#ifdef __USE_KEIL_
extern unsigned char Image$$STATIC_THREAD_REGION$$Base;
extern unsigned char Image$$STATIC_THREAD_REGION$$Length;

struct _static_thread_data* _static_thread_data_list_start;
struct _static_thread_data* _static_thread_data_list_end;
#else
...
void k_thread_foreach(k_thread_user_cb_t user_cb, void *user_data)
{
	 ...
	 */
	key = irq_lock();
	for (thread = g_kernel.threads; thread; thread = thread->next_thread) {
...
void _thread_monitor_exit(struct k_thread *thread)
{
	...
	if (thread == g_kernel.threads) {
		g_kernel.threads = g_kernel.threads->next_thread;
	} else {
		struct k_thread *prev_thread;

		prev_thread = g_kernel.threads;
		       ...
		       int prio, u32_t options)
{
	...
	new_thread->next_thread = g_kernel.threads;
	g_kernel.threads = new_thread;
	irq_unlock(key);
#endif
#ifdef CONFIG_USERSPACE
...
#ifdef __USE_KEIL_
	_static_thread_data_list_start = (struct _static_thread_data*)&Image$$STATIC_THREAD_REGION$$Base;
	_static_thread_data_list_end = (struct _static_thread_data*)(((uint32_t)&Image$$STATIC_THREAD_REGION$$Base) + ((uint32_t)&Image$$STATIC_THREAD_REGION$$Length));
#endif
	struct _static_thread_data *thread_data;  
