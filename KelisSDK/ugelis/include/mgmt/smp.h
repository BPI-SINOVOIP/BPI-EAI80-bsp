...
#ifndef H_UGELIS_SMP_
#define H_UGELIS_SMP_

#ifdef __cplusplus
extern "C" {
#endif

struct ugelis_smp_transport;
struct net_buf;

/** @typedef ugelis_smp_transport_out_fn
 * @brief SMP transmit function for uGelis.
 *
 ...
 */
typedef int ugelis_smp_transport_out_fn(struct ugelis_smp_transport *zst,
					struct net_buf *nb);

/** @typedef ugelis_smp_transport_get_mtu_fn
 * @brief SMP MTU query function for uGelis.
 *
 ...
 */
typedef uint16_t ugelis_smp_transport_get_mtu_fn(const struct net_buf *nb);

/** @typedef ugelis_smp_transport_ud_copy_fn
 * @brief SMP copy buffer user_data function for uGelis.
 *
 ...
 * @return                      0 on success, MGMT_ERR_[...] code on failure.
 */
typedef int ugelis_smp_transport_ud_copy_fn(struct net_buf *dst,
					    const struct net_buf *src);

/** @typedef ugelis_smp_transport_ud_free_fn
 * @brief SMP free buffer user_data function for uGelis.
 *
 ...
 */
typedef void ugelis_smp_transport_ud_free_fn(void *ud);

/**
 * @brief Provides uGelis-specific functionality for sending SMP responses.
 */
struct ugelis_smp_transport {
	...
	ugelis_smp_transport_out_fn *zst_output;
	ugelis_smp_transport_get_mtu_fn *zst_get_mtu;
	ugelis_smp_transport_ud_copy_fn *zst_ud_copy;
	ugelis_smp_transport_ud_free_fn *zst_ud_free;
};

/**
 * @brief Initializes a uGelis SMP transport object.
 *
 ...
 * @return                      0 on success, MGMT_ERR_[...] code on failure.
 */
void ugelis_smp_transport_init(struct ugelis_smp_transport *zst,
 ...
 */
void ugelis_smp_rx_req(struct ugelis_smp_transport *zst, struct net_buf *nb);
