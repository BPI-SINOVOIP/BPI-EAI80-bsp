...
#ifdef __USE_KEIL_
    /**
     *for kelis config bug temp +todo modify config ok
     * foce keilis to chose little endian for temp
     */
    union
    {
        /*little endian*/
...
#define _K_QUEUE_INITIALIZER(obj) \
    { \
        .data_q = SYS_SLIST_STATIC_INIT(&obj.data_q), \
                  .wait_u.wait_q = _WAIT_Q_INIT(&obj.wait_u.wait_q), \
...
/*add for ext mm*/
extern void *malloc_ext(uint8_t memx, uint32_t size);
 ...
 *
 * This should be called when the uGelis kernel has encountered an
 ...
 * defined here for special-purpose apps which want uGelis running on
