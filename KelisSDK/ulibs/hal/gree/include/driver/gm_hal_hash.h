...
#ifndef _GM_HAL_HASH_H_
#define _GM_HAL_HASH_H_
...
typedef struct HASH_Handle
{
    ...
    uint8_t               *paddingBuf;        /**< Pointter to padding buffer. This is only used when the message length is multiple blocks */
    uint32_t              paddingLength;      /**< Padding length. This is only used when the message length is multiple blocks */
