 ...
 * @date          2019/11/28
 * @note
 *    2019/11/24, Steven.lee, V1.0.0
 *        Initial version.
  *    2020/7/3, Steven.lee, V2.0.0
 *        enhance version.
 */
...
static RET_CODE HAL_SQI_IS25_DMA_4LineRead(SQI_Handle_T *hSQI, uint32_t offset, void *buff, uint32_t length)
{
    SQI_Device_T *pSQIDev;
    RET_CODE ret = 0;
    uint32_t no_4bit_num_end = 0;
    ...
    ret = HAL_SQI_IS25_4LineRead_Enable(pSQIDev, length, hSQI->flashsize);
    if (ret != RET_OK)
    {
        return RET_ERROR;
    }
 ...
 * @retval read statues
 */
static RET_CODE HAL_SQI_IS25_4Line_Read(SQI_Handle_T *hSQI, uint32_t offset, uint8_t *buffer, uint32_t size)
{
    ...
    if (hSQI->pConfig->read_mode == SQI_READ_BY_CPU)
    {
        ...
        ret = HAL_SQI_IS25_4LineRead_Enable(pSQIDev, size, hSQI->flashsize);
        if (ret != RET_OK)
        {
            return ret;
        }
    ...
    {
        ret = HAL_SQI_IS25_DMA_4LineRead(hSQI, offset, buffer, size);
        if (ret != RET_OK)
        {
            return ret;
        }
 ...
 * @retval HAL status
 */
static RET_CODE HAL_SQI_IS25_4Line_Write(SQI_Handle_T *hSQI, uint32_t offset, const void *buffer, uint32_t len)
{
    RET_CODE        ret;
    uint32_t            *tmpbuf;
    uint32_t            *tmpaddr ;
    ...
    if ((len < startpagebyte))
    {
        ret = HAL_SQI_IS25_4LineWrite_Enable(pSQIDev, len, hSQI->flashsize);
        if (ret != RET_OK)
        {
            return ret;
        }
    ...
    if ((len - startpagebyte) < hSQI->pagesize)
    {
        ...
        if (startpagebyte)
        {
            ret = HAL_SQI_IS25_4LineWrite_Enable(pSQIDev, startpagebyte, hSQI->flashsize);
            if (ret != RET_OK)
            {
                return ret;
            }
        ...
        ret = HAL_SQI_IS25_4LineWrite_Enable(pSQIDev, (len - startpagebyte), hSQI->flashsize);
        if (ret != RET_OK)
        {
            return ret;
        }
    ...
    else
    {
        ...
        while (pagecnt > 0)
        {
            ret = HAL_SQI_IS25_4LineWrite_Enable(pSQIDev, hSQI->pagesize, hSQI->flashsize);
            if (ret != RET_OK)
            {
                return ret;
            }
        ...
        if (endpagebyte)
        {
            ret = HAL_SQI_IS25_4LineWrite_Enable(pSQIDev, endpagebyte, hSQI->flashsize);
            if (ret != RET_OK)
            {
                return ret;
            }
...
/*============================================Public Function=========================================================*/
/**
 * @brief  SQI Flash standard write
 * @param  hSQI:    SQI Device Handle.
 * @param  offset:   addres offset
 * @param  buffer:   read data buff
 * @param  size:     read size
...
#ifdef CONFIG_DATA_V0
    HAL_DCACHE_Disable();
    memcpy(buffer, (void *)(CONFIG_SQI_SFLASH_MMAP_BASE + offset), len);
    HAL_SQI_Wait_Idle(pSQIDev);
    HAL_DCACHE_Enable();
#else
    memcpy(buffer, (void *)(CONFIG_SQI_SFLASH_MMAP_BASE + offset), len);
...
static RET_CODE HAL_SQI_Model_Identification(SQI_Handle_T *hSQI, uint8_t mt_id)
{
    ...
    else if (hSQI->Manufacturer == SQI_IS)
    {
        if (mt_id == IS_MT_ID)
        {
            if (hSQI->flashsize == SQI_1M_SIZE)
            {
                hSQI->model = IS25LP080D;
                hSQI->pagesize = SQI_256B_PAGE_SIZE;
            }
            else if (hSQI->flashsize == SQI_16M_SIZE)
            {
                hSQI->model = IS25LP128;
                hSQI->pagesize = SQI_256B_PAGE_SIZE;
            }
            else if (hSQI->flashsize == SQI_32M_SIZE)
            {
                hSQI->model = IS25LP256D;
                hSQI->pagesize = SQI_256B_PAGE_SIZE;
            }
        }
        else
...
static RET_CODE HAL_SQI_Device_Init(SQI_Handle_T *hSQI)
{
    ...
    else if (mf_id == IS_MF_ID)
    {
        hSQI->Manufacturer = SQI_IS;
        if ((md_id >= 0x14) && (md_id <= 0x19))
        {
            hSQI->flashsize    = sqi_is_size[md_id - 0x14];
        }
        else
        {
            printf("#############Sorry,The flash model is don't support!#############\n");
...
static RET_CODE HAL_SQI_4Address_Enable(SQI_Handle_T *hSQI)
{
    ...
    {
        return HAL_SQI_IS25_4Address_Enable(pSQIDev);
    }
    return RET_OK;
}
 ...
 * @param  hSQI:        SQI Device Handle.
 * @param  pSQIDev         the device address of sflash control
 * @param  pConfig   The config for sflash ,include read mode ,bit mode and clock div
...
RET_CODE HAL_SQI_Read(SQI_Handle_T *hSQI, uint32_t offset, void *buff, uint32_t length)
{
    ...
    if (hSQI)
    {
        ...
        if (hSQI->pConfig->bit_mode == SQI_4BIT_MODE)
        {
            ...
            {
                return HAL_SQI_IS25_4Line_Read(hSQI, offset, buff, length);
            }
        }
        else
        {
...
RET_CODE HAL_SQI_Write(SQI_Handle_T *hSQI, uint32_t offset, const void *buff, uint32_t length)
{
    ...
    if (hSQI)
    {
        ...
        if (hSQI->pConfig->bit_mode == SQI_4BIT_MODE)
        {
            ...
            {
                return HAL_SQI_IS25_4Line_Write(hSQI, offset, buff, length);
            }
        }
        else
        {
...
/**
 * @brief  SQI Flash  erase
 * @param  hSQI:    SQI Device Handle.
 * @param  offset:   addres offset
 * @param  buff:   read data buff
 * @param  length:     earse size
 * @retval RET_ERROR      There is invalid configuration not supported
 * @retval RET_OK         Configure successfully
 */
...
/**
 * @brief  SQI Flash  write,this function can write flash without erase
 * @param  hSQI:    SQI Device Handle.
 * @param  offset:   addres offset
 * @param  buff:   read data buff
 * @param  length:     write size
 ...
 * @param  hSQI:    SQI Device Handle.
 * @retval -1           get flash size failed
 * @retval flash size flash size
 */
 ...
 * @param  hSQI:    SQI Device Handle.
 * @retval -1           get flash size failed
 * @retval flash page size flash page size
 */
