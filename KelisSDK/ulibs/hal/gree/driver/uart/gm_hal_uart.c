...
void HAL_UART_IRQHandler(void *param)
{
    ...
    if (LL_UART_Is_IE_RFTO(pUART) && LL_UART_Is_IS_RFTO(pUART))
    {
        UART_RxInterruptHandler(pUART, hUART, callback, TRUE);
    }

    /* NonFifo && Received Data Available */
    if (LL_UART_Is_IE_RXDA(pUART) && LL_UART_Is_IS_RXDA(pUART))
    {
    ...
    if (LL_UART_Is_IE_RFT(pUART) && LL_UART_Is_IS_RFT(pUART))
    {
        LL_UART_Clear_IS_RFT(pUART);
        UART_RxInterruptHandler(pUART, hUART, callback, FALSE);
    }
    ...
    if (LL_UART_Is_IE_TFT(pUART) && LL_UART_Is_IS_TFT(pUART))
    {
        LL_UART_Clear_IS_TFT(pUART);
        UART_TxInterruptHandler(pUART, hUART, callback);
    }

    // UART_EVENT_TX_FINISHED to tell tx data finished transmit, then we can close tx by setmode(rx)
    if (LL_UART_Is_IE_TXIDL(pUART) && LL_UART_Is_IS_TXIDL(pUART))
    {
        LL_UART_Clear_IS_TXIDL(pUART);
        if (NULL != hUART->callback)
        {
            hUART->callback(hUART, UART_EVENT_TX_FINISHED, (ulong_t)0);
        }
    }
}
