...
static void HASH_DMACallback(struct DMA_Handle *hDma, uint8_t event, unsigned long param)
{
    ...
    {
        if (tmpHandle->paddingBuf != NULL)
        {
            HAL_DMA_Start(tmpHandle->hdmain, (uint32_t)tmpHandle->paddingBuf, (uint32_t) & (tmpHandle->Instance->H_DIN), tmpHandle->paddingLength / 4);
            tmpHandle->paddingBuf = NULL;
        }
        else
        {
...
void HAL_HASH_IRQHandler_Func(void **handle)
{
    ...
    if (HAL_HASH_GET_ITSTATUS(hHash, HASH_INTST_DININT_SET))
    {
        ...
        else
        {
            if (tranSize == tranMax)
            {
                hHash->pHashInBuffPtr = hHash->paddingBuf;
                inputAddr = (uint32_t)(hHash->pHashInBuffPtr);
            }
            ...
            if (tranSize == tranMax)
            {
                ...
                k_free(hHash->paddingBuf);
#else
                free(hHash->paddingBuf);
#endif
            }
...
HAL_Status_T HAL_HASH_Start(HASH_Handle_T *hHash, uint8_t *pInBuffer, uint32_t size)
{
    ...
    if (hHash->state == HASH_STATE_READY)
    {
        ...
        /* fix hardware bug: when the message length is multiple blocks, hardware could not padding message, it is wrong */
        if (size % 64 == 0)
        {
            uint32_t tmp = 0;
            if (hHash->init->algo == HASH_ALGO_SHA1 || hHash->init->algo == HASH_ALGO_SHA224 || hHash->init->algo == HASH_ALGO_SHA256)
            {
#ifndef RABBIT_NO_OS_SYSTEM
                hHash->paddingBuf = (uint8_t *)k_malloc(64);
#else
                hHash->paddingBuf = (uint8_t *)malloc(64);
#endif
                hHash->paddingLength = 64;
                memset(hHash->paddingBuf, 0, 64);
                hHash->paddingBuf[0] = 0x80;
                tmp = (size * 8) & 0xFFFFFFFF;
                *(volatile uint32_t *)(&hHash->paddingBuf[60]) = __REV(tmp);
                tmp = ((uint64_t)(size * 8) >> 32);
                *(volatile uint32_t *)(&hHash->paddingBuf[56]) = __REV(tmp);
                size += 64;
            }
        }
        ...
        if (size % 128 == 0)
        {
            ...
            {
#ifndef RABBIT_NO_OS_SYSTEM
                hHash->paddingBuf = (uint8_t *)k_malloc(128);
#else
                hHash->paddingBuf = (uint8_t *)malloc(128);
#endif
                hHash->paddingLength = 128;
                memset(hHash->paddingBuf, 0, 128);
                hHash->paddingBuf[0] = 0x80;
                tmp = (size * 8) & 0xFFFFFFFF;
                *(volatile uint32_t *)(&hHash->paddingBuf[124]) = __REV(tmp);
                tmp = ((uint64_t)(size * 8) >> 32);
                *(volatile uint32_t *)(&hHash->paddingBuf[120]) = __REV(tmp);
                size += 128;
            }
        }
        ...
        if (hHash->init->enableDMA == TRUE)
        {
            hHash->hdmain->callback = HASH_DMACallback;
            /* Enable DMA requests */
            if (hHash->paddingBuf != NULL)
            {
                if (hHash->init->algo == HASH_ALGO_SHA1 || hHash->init->algo == HASH_ALGO_SHA224 || hHash->init->algo == HASH_ALGO_SHA256)
                {
                    tranSize -= 16;
                }
                else if (hHash->init->algo == HASH_ALGO_SHA384 || hHash->init->algo == HASH_ALGO_SHA512)
                {
                    tranSize -= 32;
                }
            }
