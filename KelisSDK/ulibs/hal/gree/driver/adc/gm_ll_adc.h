...
#define ADC_DEV0                              ((ADC_Device_T *)ADC0_BASE) /* ADC device pointer */
#define ADC_DEV1                              ((ADC_Device_T *)ADC1_BASE) /* ADC device pointer */
#define ADC_DEV2                              ((ADC_Device_T *)ADC2_BASE) /* ADC device pointer */

#define ADC1                             	  ((ADC_Device_T *)ADC0_BASE)/* Base addresses of ADC registers */
#define ADC2                                  ((ADC_Device_T *)ADC1_BASE)/* Base addresses of ADC registers */
#define ADC3                                  ((ADC_Device_T *)ADC2_BASE)/* Base addresses of ADC registers */
#define ADC_COM_DEV                           ((ADC_Common_TypeDef *)ADC_COM_BASE) /* Common base addresses of ADC registers*/

#define ADC_READ_FIFO_OFFSET                  0x10   /* ADC read fifo offset To ADC reg base */
#define ADC0_READ_FIFO_PHY_ADDRESS            (uint32_t)((uint32_t)ADC_DEV0 + ADC_READ_FIFO_OFFSET) /* ADC read fifo offset physical address */
#define ADC1_READ_FIFO_PHY_ADDRESS            (uint32_t)((uint32_t)ADC_DEV1 + ADC_READ_FIFO_OFFSET) /* ADC read fifo offset physical address */
#define ADC2_READ_FIFO_PHY_ADDRESS            (uint32_t)((uint32_t)ADC_DEV2 + ADC_READ_FIFO_OFFSET) /* ADC read fifo offset physical address */
...
#define ADC_JEOS_INTEN                        ((uint32_t) ((uint32_t)1 << ADC_JEOS_INTEN_POS))     /* ADC Interrupt enable for injected channel end of sequence filed */

#define ADC_JEOC_INTEN_POS                    ((uint32_t) 24)  /* ADC Interrupt enable for injected channel end of conversiont enable pos */
#define ADC_JEOC_INTEN                        ((uint32_t) ((uint32_t)1 << ADC_JEOC_INTEN_POS))     /* ADC  Interrupt enable for injected channel end of conversion filed */
...
#define ADCX_JCHN3_SEL                       ((uint32_t) ((uint32_t)0X1F << ADCX_JCHN3_SEL_POS))     /* ADC Channel select bits of 4th conversion in injected sequence filed */
...
/**
  ...
  * @retval None
  */
#define LL_ADC_ENABLE(__HANDLE__) ((__HANDLE__)->device->CTRL |=  ADC_CTRL_EN)
...
/**
  ...
  * @retval None
  */
#define __HAL_ADC_DISABLE(__HANDLE__) ((__HANDLE__)->device->CTRL &=  ~ADC_CTRL_EN)
...
/**
  ...
  * @retval None
  */
#define LL_ADC_ENABLE_IT(__HANDLE__, __INTERRUPT__) (((__HANDLE__)->device->IE) |= (__INTERRUPT__))
...
/**
  ...
  * @retval None
  */
#define __HAL_ADC_DISABLE_IT(__HANDLE__, __INTERRUPT__) (((__HANDLE__)->device->IE) &= ~(__INTERRUPT__))
...
/** @brief  Check if the specified ADC interrupt source is enabled or disabled.
  ...
  */
#define __HAL_ADC_GET_IT_SOURCE(__HANDLE__, __INTERRUPT__)  (((__HANDLE__)->device->IE & (__INTERRUPT__)) == (__INTERRUPT__))
...
/**
  ...
  * @retval None
  */
#define __HAL_ADC_CLEAR_FLAG(__HANDLE__, __FLAG__) (((__HANDLE__)->device->ST) = (__FLAG__))
...
/**
  ...
  * @retval None
  */
#define __HAL_ADC_GET_FLAG(__HANDLE__, __FLAG__) ((((__HANDLE__)->device->ST) & (__FLAG__)) == (__FLAG__))
 ...
 * @retval  None
 */
#define ADC_COFNIG_HW_JTRIGER_SOUTCE(pADC, value)               IO_BITMASK_SET(pADC->HWTRIGCTR, ADC_JHTRIG_SRCSEL,\
                                                                            value << ADC_JHTRIG_SRCSEL_POS)
 ...
 * @retval   None
 */
#define ADC_COFNIG_HW_JTRIGER_EN(pADC, value)                    IO_BITMASK_SET(pADC->HWTRIGCTR, ADC_JHTRIG_EN,\
                                                                           value << ADC_JHTRIG_EN_POS)
...
#define ADC_INJECTED_RANK_3    ((uint32_t)0x00000003)
 ...
 #define __HAL_LOCK(__HANDLE__)                                           \
                                do{                                        \
                                    if((__HANDLE__)->Lock == HAL_ADC_LOCKED)   \
                                    {                                      \
                                       return HAL_BUSY;                    \
                                    }                                      \
                                    else                                   \
                                    {                                      \
                                       (__HANDLE__)->Lock = HAL_ADC_LOCKED;    \
                                    }                                      \
                                  }while (0U)

#define __HAL_UNLOCK(__HANDLE__)                                          \
                                  do{                                       \
                                      (__HANDLE__)->Lock = HAL_ADC_UNLOCKED;    \
                                    }while (0U)
