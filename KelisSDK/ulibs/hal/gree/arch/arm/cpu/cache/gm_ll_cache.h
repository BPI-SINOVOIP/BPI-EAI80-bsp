...
#ifdef CHIP_F6721B
typedef CMSDK_DCACHE_TypeDef CMSDK_DBUSCACHE_TypeDef;
#define CMSDK_DBUSCACHE                         ((CMSDK_DBUSCACHE_TypeDef*) (0xE0042000 + 0x200))
#endif
...
#ifdef CHIP_F6721B
#define DBUSCACH_SIZE                            (8*1024)
#define DBUSACHE_PER_LINE_SIZE                  16
#define DBUSCACHE_LINE_NUM                      (DBUSCACH_SIZE/DBUSACHE_PER_LINE_SIZE)
#endif
...
#ifdef CHIP_F6721B
#define HAL_DBUSACHE_INVALID_DATA_ALL_LINE()       do{                                                       \
        register uint32_t n;                                 \
        for(n=0;n<DBUSCACHE_LINE_NUM;n++) {                  \
            CMSDK_DBUSCACHE->DCBIR = n*DBUSACHE_PER_LINE_SIZE ;  \
        }                                                    \
    }while(0)
#endif
...
#ifdef CHIP_F6721B
#define HAL_DBUSACHE_INVALID_DATA_LINE(i)       CMSDK_DBUSCACHE->DCBIR = (i)*DBUSACHE_PER_LINE_SIZE
#endif
...
#ifdef CHIP_F6721B
#define LL_DBUSCACHE_BYPASS_POS                 ((uint32_t) 5)
#define LL_DBUSCACHE_BYPASS_MASK                ((uint32_t) ((uint32_t)1 << LL_DBUSCACHE_BYPASS_POS))
#endif
...
#ifdef CHIP_F6721B
#define LL_DBUSCACHE_SET_BYPASS(pDev,val)          IO_BITMASK_SET(pDev->CACHECTR,LL_DBUSCACHE_BYPASS_MASK ,(!val)<<LL_DBUSCACHE_BYPASS_POS)
#endif
